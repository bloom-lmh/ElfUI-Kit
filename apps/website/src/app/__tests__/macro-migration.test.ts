...
const kitSourceRoot = join(process.cwd(), "packages", "kit", "src");
const websiteSourceRoot = join(process.cwd(), "apps", "website", "src");
const componentsRoot = join(kitSourceRoot, "components");
...
const sourceFiles = [
  ...collectSourceFiles(kitSourceRoot),
  ...collectSourceFiles(websiteSourceRoot),
];
const componentFiles = collectSourceFiles(componentsRoot);

const toRelativePath = (file: string): string => relative(process.cwd(), file).replace(/\\/g, "/");
...
import { Button } from "../../../../../packages/kit/src/components/Basic/Button/index";
import { Badge } from "../../../../../packages/kit/src/components/Basic/Badge/index";

useComponents(Button, Badge);

export const Probe = defineHtml(\`
...
\`);
      `,
      {
        filename: join(websiteSourceRoot, "app", "__tests__", "local-component-probe.ts"),
