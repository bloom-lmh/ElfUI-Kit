...
const t = createDocsTranslator({
  ...
  underline: {
    zh: "默认、悬停与焦点时是否允许下划线",
    en: "Whether underline is allowed at rest, hover, and focus",
  },
  disabled: {
    zh: "禁用导航并退出 Tab 顺序",
    en: "Disables navigation and removes the link from the Tab order",
  },
  href: {
    zh: "原生链接地址；to 存在时作为次要回退",
    en: "Native URL; secondary fallback when to is present",
  },
  to: {
    zh: "ElfUI Router 目标，优先于 href",
    en: "ElfUI Router destination; takes precedence over href",
  },
  replace: {
    zh: "使用 router.replace 而非 router.push",
    en: "Uses router.replace instead of router.push",
  },
  target: {
    zh: "原生浏览上下文；_blank 自动补安全 rel",
    en: "Native browsing context; _blank adds a safe rel",
  },
  rel: {
    zh: "链接关系 token；显式值会与安全默认值合并",
    en: "Relationship tokens merged with safe defaults",
  },
  activeClass: {
    zh: "路由前缀匹配时的内部链接 class",
    en: "Internal anchor class for an active route",
  },
  exactActiveClass: {
    zh: "路由精确匹配时的内部链接 class",
    en: "Internal anchor class for an exact route",
  },
  icon: {
    zh: "简短文本图标；icon 插槽优先",
    en: "Short text icon; the icon slot takes precedence",
  },
  navigate: {
    zh: "客户端路由导航开始时触发，detail 为 to",
    en: "Emitted when client-side router navigation starts; detail is to",
  },
  content: { zh: "链接内容", en: "Link content" },
  iconSlot: {
    zh: "自定义图标，优先于 icon 属性",
    en: "Custom icon; takes precedence over the icon property",
  },
});
...
const propsRows = () => [
  ...
  { name: "active-class", type: "string", default: "router default", desc: t("activeClass") },
  {
    ...
    desc: t("exactActiveClass"),
  },
  { name: "icon", type: "string", default: "''", desc: t("icon") },
];

const eventsRows = () => [{ name: "navigate", desc: t("navigate") }];
