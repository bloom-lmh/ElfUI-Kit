...
describe("Tag documentation", () => {
  ...
  it("adds, edits, and removes a dynamic tag without putting edit state into Tag", async () => {
    ...
    page
      .shadowRoot!.querySelector<HTMLFormElement>(".tag-add-form")!
      .dispatchEvent(new SubmitEvent("submit", { bubbles: true, cancelable: true }));
    await tick();

    const added = Array.from(
      page.shadowRoot!.querySelectorAll<HTMLElement>(".tag-editor-list elf-tag"),
    ...
    await tick();
    expect(page.shadowRoot!.querySelector(".tag-editor-list")?.textContent).toContain(
      "Design tokens",
    );

    added.shadowRoot!.querySelector<HTMLButtonElement>(".close")!.click();
    await tick();
    expect(page.shadowRoot!.querySelector(".tag-editor-list")?.textContent).not.toContain(
      "Release",
    );
  });
});
