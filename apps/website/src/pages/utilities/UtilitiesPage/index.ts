...
const t = createDocsTranslator({
  ...
  clearfixDescription: {
    zh: "clearfix 让容器正确包住内部浮动元素。",
    en: "clearfix makes the container wrap its floated children correctly.",
  },
  ...
  editorialDescription: {
    zh: "这个示例展示编辑内容如何环绕浮动图片，同时保持自然阅读顺序。",
    en: "A focused example shows how editorial content wraps around a floated visual while the reading flow stays intact.",
  },
  ...
  publishing: {
    zh: "正在发布包含超长状态文本的候选版本",
    en: "publishing release candidate with a deliberately long status line",
  },
  ...
  typographyDescription: {
    zh: "排版在装饰介入之前建立信息层级。",
    en: "Typography builds hierarchy before decoration enters the page.",
  },
...
const createInitialLabs = (): Record<UtilityKey, UtilityLabState> =>
  Object.fromEntries(entries.map(({ key, category }) => [key, defaultState(category)])) as Record<
    UtilityKey,
    UtilityLabState
  >;
...
const isSelected = (key: UtilityKey, className: string): boolean =>
  selectedClass(key) === className;

const floatContainerClass = (): string => (selectedClass("float") === "clearfix" ? "clearfix" : "");

const floatMediaClass = (): string =>
  selectedClass("float") === "clearfix" ? "float-start" : selectedClass("float");
...
const contentExplanation = (): string =>
  [
    pick(
      ...
      "Accessibility classes preserve skip links or supplemental descriptions for screen readers and can reveal them on keyboard focus.",
    ),
    pick(
      "指针事件类决定覆盖层是否拦截鼠标；pass-through 适合可穿透遮罩。",
      "Pointer-event classes determine whether an overlay intercepts the pointer; pass-through is suited to transparent masks.",
    ),
    pick(
      "内容适配类控制图片裁切或完整显示；clearfix 用于包住浮动内容。",
      "Content-fitting classes crop or fully display images; clearfix wraps floated content.",
    ),
...
const displayExplanation = (): string =>
  [
    pick(
      ...
      "Display classes switch an element's layout type and can apply from a selected responsive breakpoint.",
    ),
    pick(
      "条件隐藏类按视口范围隐藏内容；print-only 与 screen-only 用于区分屏幕和打印环境。",
      "Conditional visibility classes hide content by viewport range; print-only and screen-only distinguish screen and print output.",
    ),
    pick(
      "打印类只在浏览器打印或导出 PDF 时改变元素的 display，不影响普通屏幕布局。",
      "Print classes change display only during browser printing or PDF export without affecting the regular screen layout.",
    ),
...
const contentControlExplanation = (): string => {
  const descriptions: Record<string, string> = {
    "d-sr-only": pick(
      "仅向屏幕阅读器提供内容，视觉界面中保持隐藏。",
      "Provides content only to screen readers while keeping it visually hidden.",
    ),
    "d-sr-only-focusable": pick(
      "默认视觉隐藏，键盘聚焦时显示跳转入口。",
      "Visually hidden by default and reveals the skip entry on keyboard focus.",
    ),
    "visually-hidden": pick(
      "隐藏视觉内容，同时保留辅助技术可读取的语义。",
      "Hides visual content while preserving semantics for assistive technology.",
    ),
    "pointer-events-none": pick(
      "覆盖层不拦截鼠标事件，事件会落到其下方元素。",
      "The overlay does not intercept pointer events, so they reach the element below.",
    ),
    "pointer-events-auto": pick(
      "覆盖层按默认规则接收并拦截鼠标事件。",
      "The overlay receives and intercepts pointer events using default behavior.",
    ),
    "pointer-pass-through": pick(
      "覆盖层本身可穿透，但其中明确可交互的子元素仍能操作。",
      "The overlay itself passes events through while explicitly interactive children remain operable.",
    ),
    "object-cover": pick(
      "图片覆盖容器，必要时裁切超出的内容。",
      "The image covers its container and crops overflow when needed.",
    ),
    "object-contain": pick(
      "完整显示图片，并在比例不一致时保留空白。",
      "The complete image remains visible, with empty space when aspect ratios differ.",
    ),
    clearfix: pick(
      "清除内部浮动对父容器高度计算造成的影响。",
      "Clears the effect of internal floats on the parent container's calculated height.",
    ),
  };
  return (
    descriptions[selectedClass("content")] ||
    pick(
      "内容工具类用于控制辅助语义、指针事件和媒体适配。",
      "Content utilities control assistive semantics, pointer events, and media fitting.",
    )
  );
};
...
const displayControlExplanation = (): string => {
  ...
  const descriptions: Record<string, string> = {
    "d-none": pick(
      "隐藏元素，不占据页面布局空间。",
      "Hides the element without occupying layout space.",
    ),
    "d-flex": pick(
      "将元素切换为 Flex 布局容器。",
      "Switches the element to a Flex layout container.",
    ),
    "d-md-inline-flex": pick(
      "从 md 断点开始使用 inline-flex 布局。",
      "Uses inline-flex from the md breakpoint upward.",
    ),
    "d-xxl-table": pick(
      "从 xxl 断点开始使用 table 布局。",
      "Uses table layout from the xxl breakpoint upward.",
    ),
    "hidden-xs": pick(
      "仅在 xs 视口范围隐藏元素。",
      "Hides the element only in the xs viewport range.",
    ),
    "hidden-md-and-up": pick(
      "在 md 及更宽视口隐藏元素。",
      "Hides the element at md and wider viewports.",
    ),
    "hidden-lg-and-down": pick(
      "在 lg 及更窄视口隐藏元素。",
      "Hides the element at lg and narrower viewports.",
    ),
    "hidden-print-only": pick(
      "打印或导出 PDF 时隐藏，屏幕上保持可见。",
      "Hides during printing or PDF export while remaining visible on screen.",
    ),
  };
