...
describe("UtilitiesPage", () => {
  it("中文页面渲染全部工具类交互面板、运行文案和源码", async () => {
    ...
    expect(
      root.querySelector<HTMLElement>("#utility-typography elf-playground")?.shadowRoot
        ?.textContent,
    ).toContain("文本和排版");
  });
  ...
  it("为不同工具主题提供不同的语义化预览", async () => {
    ...
    expect(root.querySelector("#utility-content .config-purpose")?.textContent).toContain(
      "屏幕阅读器",
    );
    expect(root.querySelector("#utility-display .config-purpose")?.textContent).toContain(
      "隐藏元素",
    );
  });
  ...
  it("updates content and display purpose copy from the selected utility class", async () => {
    ...
    contentSelects[1]!.dispatchEvent(
      new CustomEvent("update:modelValue", { detail: "pointer-pass-through" }),
    );
    await tick();
  ...
  it("documents reusable drag sources, drop targets and sorting callbacks", async () => {
    ...
    const playground = draggable.shadowRoot!.querySelector<HTMLElement & { script?: string }>(
      "elf-playground",
    )!;
    ...
    expect(
      draggable.shadowRoot!.querySelector(".archive-target")?.hasAttribute("data-droppable"),
    ).toBe(true);
    expect(draggable.shadowRoot!.querySelector(".task-card")?.hasAttribute("data-draggable")).toBe(
      true,
    );
  });
});
