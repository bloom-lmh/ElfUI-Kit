...
import { BREAKPOINTS, CATALOG } from "./catalog";

const utilitySource = readFileSync(
  resolve(process.cwd(), "packages/kit/src/styles/utilities.scss"),
  "utf8",
);
const utilityStyles = compileString(utilitySource, {
  url: pathToFileURL(resolve(process.cwd(), "packages/kit/src/styles/utilities.scss")),
}).css;
