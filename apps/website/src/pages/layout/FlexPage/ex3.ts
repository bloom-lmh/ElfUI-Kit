...
const wrapOptions = () =>
  ["nowrap", "wrap", "wrap-reverse"].map((value) => ({ label: value, value }));
...
const alignContentCode = (): string =>
  `<elf-flex wrap align-content="${alignContentMode.value}" gap="sm">1 2 3</elf-flex>`;
const compatibilityCode = (): string =>
  compatibilityMode.value === "fill"
