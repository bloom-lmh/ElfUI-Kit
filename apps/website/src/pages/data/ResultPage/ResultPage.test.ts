...
describe("ResultPage locale", () => {
  it("renders Chinese docs", async () => {
    ...
    expect(text).toContain("四种状态");
    expect(text).toContain("扩展操作区");
    expect(text).toContain("替换默认状态图形");
  });
  it("renders strict English docs", async () => {
    document.documentElement.lang = "en-US";
    const text = await mount();
    expect(text).toContain("Four states");
    expect(text).toContain("Extra actions");
    expect(text).toContain("Replace the default status graphic");
