...
const t = createDocsTranslator({
  ...
  distance: {
    zh: "距滚动底部多少像素时触发",
    en: "Distance from the scroll bottom that triggers loading",
  },
  delay: {
    zh: "滚动触发的合并延迟，负数归一为 0",
    en: "Coalescing delay; negative values normalize to 0",
  },
  loading: {
    zh: "请求进行中时阻止重复 load",
    en: "Prevent duplicate load events while a request is active",
  },
  disabled: {
    zh: "暂停加载，适合错误或临时禁用状态",
    en: "Pause loading for errors or temporary disabled states",
  },
  finished: {
    zh: "明确标记没有更多数据并永久停止加载",
    en: "Mark the data source complete and stop further loading",
  },
  immediate: {
  ...
  load: {
    zh: "接近阈值且未禁用、加载或完成时触发",
    en: "Emitted near the threshold when active, idle, and unfinished",
  },
  defaultSlot: {
    zh: "信息流内容及调用方管理的状态反馈",
    en: "Feed content and consumer-owned status feedback",
  },
  check: {
    zh: "立即重新测量当前容器并检查阈值",
    en: "Measure the current container and check the threshold immediately",
  },
  directive: {
    zh: "接近任意滚动元素底部时执行处理函数",
    en: "Run a handler near the bottom of any scroll element",
  },
...
const eventRows = () => [{ name: "load", type: "() => void", desc: t("load") }];

const slotRows = () => [{ name: "default", desc: t("defaultSlot") }];

const exposeRows = () => [{ name: "check()", desc: t("check") }];
