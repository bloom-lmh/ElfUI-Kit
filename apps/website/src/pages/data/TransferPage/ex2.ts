...
const t = createDocsTranslator({
  ...
  playground: {
    zh: "自定义字段、禁用项、过滤方法与前置排序",
    en: "Custom fields, disabled items, custom filtering, and unshift ordering",
  },
...
const readKeys = (event: Event): string[] =>
  Array.isArray((event as CustomEvent).detail) ? (event as CustomEvent).detail : [];
const onTransfer = (event: Event): void => selected.set(readKeys(event));
const filterUsers = (query: string, user: { name: string }): boolean =>
