...
beforeAll(async () => {
  ...
  const { PageStatistic } = await import("./index");
  pageTag = ensureCustomElement(PageStatistic);
}, 30_000);
afterEach(() => {
...
describe("StatisticPage locale", () => {
  it("renders Chinese docs", async () => {
    ...
    expect(text).toContain("基础数值");
    expect(text).toContain("自定义格式化与数值样式");
    expect(text).toContain("动态增长");
    expect(text).toContain("倒计时");
    expect(text).toContain("自定义格式化函数");
  });
  it("renders strict English docs", async () => {
    document.documentElement.lang = "en-US";
    const text = await mount();
    expect(text).toContain("Basic values");
    expect(text).toContain("Custom formatter and value style");
    expect(text).toContain("Animated growth");
    expect(text).toContain("Countdown");
    expect(text).toContain("Custom formatter");
