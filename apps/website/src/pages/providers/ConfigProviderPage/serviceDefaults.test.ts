...
const wait = (ms = 20): Promise<void> => new Promise((resolve) => window.setTimeout(resolve, ms));

const deepQueryAll = <T extends Element>(root: ParentNode, selector: string): T[] => {
...
afterEach(async () => {
  ...
  document.querySelector<HTMLElement>("elf-loading")?.dispatchEvent(new CustomEvent("close"));
  await wait(230);
...
describe("ConfigProvider service defaults", () => {
  it("scopes defaults for all service APIs while method variants stay explicit", async () => {
    ...
    buttons[2]!.click();
    await wait();
    const loading = document.body.querySelector<HTMLElement & { variant?: string }>("elf-loading");
    ...
    expect(messageBox?.shadowRoot?.querySelector(".confirm")?.textContent).toContain(
      "Leave anyway",
    );
    expect(messageBox?.shadowRoot?.querySelector(".cancel")?.textContent).toContain("Stay here");
