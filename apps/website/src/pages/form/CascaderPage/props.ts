...
const propsRows = (): ApiRow[] => [
  ...
  row(
    ...
    "-",
    "字段名、懒加载和勾选行为",
    "Field names, lazy loading, and checking behavior",
  ),
  row(
    "variant / label",
    "FieldVariant / string",
    "filled / ''",
    "字段表面与浮动标签",
    "Field surface and floating label",
  ),
  ...
  row(
    ...
    "built-in / -",
    "空值和清空值策略",
    "Empty-value and clear-value policy",
  ),
  row(
    "multiple / checkable",
    "boolean",
    "false",
    "多选和联动复选框",
    "Multiple selection and linked checkboxes",
  ),
  row(
    "panelMode / treeThreshold",
    "auto | columns | tree / number",
    "auto / 3",
    "深层路径自动使用紧凑树",
    "Automatically use a compact tree for deep paths",
  ),
  row(
    ...
    "false / true",
    "父子独立选择与路径返回",
    "Independent checking and emitted path",
  ),
  row(
    "showAllLevels / separator",
    "boolean / string",
    "true / ' / '",
    "路径回显方式",
    "Displayed path format",
  ),
  row(
  ...
  row(
    "checkOnClickNode / checkOnClickLeaf",
    "boolean",
    "false / true",
    "节点和叶子的点击选择策略",
    "Node and leaf click-selection policy",
  ),
  ...
  row(
    ...
    "false / 300",
    "路径搜索与防抖",
    "Path search and debounce",
  ),
  row(
    "filterMethod / beforeFilter",
    "function",
    "-",
    "自定义匹配和搜索前置守卫",
    "Custom matcher and pre-filter guard",
  ),
  row(
    "teleported / appendTo",
    "boolean / selector | HTMLElement",
    "true / body",
    "Top Layer 与挂载目标",
    "Top Layer and mount target",
  ),
  row("persistent", "boolean", "true", "关闭后保留面板 DOM", "Keep panel DOM after closing"),
  row(
    "placement / fallbackPlacements",
    "CascaderPlacement / CascaderPlacement[]",
    "bottom-start / built-in",
    "首选和碰撞候选方向",
    "Preferred and collision fallback placements",
  ),
  ...
  row(
    ...
    "{}",
    "偏移、翻转和溢出修饰器",
    "Offset, flip, and overflow modifiers",
  ),
  row(
    "effect / tagType / tagEffect",
    "string",
    "light / info / light",
    "面板和标签外观",
    "Panel and tag appearance",
  ),
  row(
    "validateEvent",
    "boolean",
    "true",
    "触发表单 change/blur 校验",
    "Trigger form change/blur validation",
  ),
  row(
...
const panelRows = (): ApiRow[] => [
  row(
    "modelValue / options",
    "CascaderModelValue / CascaderOption[]",
    "[] / []",
    "选择值和选项树",
    "Value and option tree",
  ),
  row(
    "multiple / checkable",
    "boolean",
    "false",
    "多选或联动复选框",
    "Multiple selection or linked checkboxes",
  ),
  row(
    "checkStrictly / emitPath / showPrefix",
    "boolean",
    "-",
    "勾选与返回值控制",
    "Checking and emitted-value control",
  ),
  row(
    "virtualScroll / itemSize / height",
    "boolean / number / number",
