...
const t = createDocsTranslator({
  ...
  inboxContent: {
    zh: "集中查看通知和待办消息。",
    en: "Review notifications and pending messages.",
  },
...
const script = (): string => `const active = useRef("reports");
...
const onAdd = () => {
  ...
  tabItems.set([...tabItems.value, { label: "${t("newTab")}", value, content: "${t("newContent")}" }]);
  active.set(value);
};
