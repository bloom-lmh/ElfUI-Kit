...
| Field, Form and clear defaults                                | `src/types/form.ts`, composables `form-context.ts` / `form.ts` / `native-form.ts` / `field-values.ts`, Provider `config.field` | Form, FormItem, Input family, Select, Cascader, Pickers, Rate, Switch, TreeSelect and Upload                     | Parallel validation contexts, direct ElementInternals calls, copied disabled/size inheritance, component-local Provider precedence       | Core owns native association; Kit owns serialization and the typed model bridge                           |
...
| Document-level portal styles                                  | `src/composables/useGlobalStyle.ts` wrapping Core `globalStyle`                                                                | Image, Dialog, Drawer, Tour and Sticky portals                                                                   | Module-load DOM access, duplicate per-instance style nodes, or global styles without lifecycle cleanup                                   | SSR-safe, reference-counted mount ownership                                                               |
