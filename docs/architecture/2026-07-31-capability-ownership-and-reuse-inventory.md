...
| Modal focus and scroll lock                                   | `modal-overlay-controller.ts`, `useModalOverlay.ts`, Core `useScrollLock`                                             | Dialog, Drawer, MessageBox; fullscreen owners should consume Core scroll lock                                    | A second global scroll-lock registry, shallow focus traps, timers for focus return                                                       | Core beta.20 is the target lock owner; Loading service remains an `OP-03` / `OP-07` migration violation |
