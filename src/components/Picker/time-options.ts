...
export const parseTimeStep = (value: unknown): number => {
  ...
  return parsed && parsed.totalMinutes > 0
    ? parsed.totalMinutes
    : DEFAULT_STEP;
};
...
): number | null => parseClockTime(value)?.totalMinutes ?? fallback;

export const createTimeOptions = (
  config: TimeOptionConfig = {},
): SelectOption[] => {
  ...
  return values.map((totalMinutes) => ({
    ...
    disabled:
      (minTime !== null && totalMinutes < minTime) ||
      (maxTime !== null && totalMinutes > maxTime),
  }));
};
