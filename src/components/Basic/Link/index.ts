...
const normalizedRel = (): string | null => {
  if (props.disabled) return null;
  const tokens = new Set(String(props.rel || "").split(/\s+/).filter(Boolean));
...
const isActive = (): boolean => {
  ...
  return currentPath === target.path || (target.path !== "/" && currentPath.startsWith(`${target.path}/`));
};
...
const linkClasses = (): string[] => {
  ...
  if (isExactActive()) classes.push(props.exactActiveClass || router?.options.linkExactActiveClass || "exact-active");
  return classes;
};
...
const onClick = (event: MouseEvent): void => {
  ...
  void (props.replace ? router.replace(props.to as RouteLocationRaw) : router.push(props.to as RouteLocationRaw));
};
