...
describe("elf-icon-provider", () => {
  ...
  it("allows a nested provider to replace the default set while inheriting aliases", async () => {
    ...
    expect(outer.querySelector("#outer-icon")!.shadowRoot!.querySelector("path")?.getAttribute("d"))
      .toBe("outer-path");
    expect(outer.querySelector("#inner-icon")!.shadowRoot!.querySelector("path")?.getAttribute("d"))
      .toBe("inner-path");
    expect(outer.querySelector("#alias-icon")!.shadowRoot!.querySelector("path")?.getAttribute("d"))
      .toBe("outer-path");
  });
});
