...
import type { TransferDataItem, TransferDirection, TransferProps, TransferRenderContext, TransferSlots } from "./types";
...
const sideItems = (side: "left" | "right"): TransferViewItem[] => side === "left" ? source.value : target.value;
const sideScrollTop = (side: "left" | "right"): number => side === "left" ? leftScrollTop.value : rightScrollTop.value;
const virtualWindow = (side: "left" | "right") => computeVirtualWindow({
...
const panelBodyStyle = (): Record<string, string> => props.virtual
  ? { height: `${numericHeight()}px`, minHeight: `${numericHeight()}px`, maxHeight: `${numericHeight()}px` }
  : {};
...
const itemStyle = (): Record<string, string> => props.virtual
  ? { height: `${numericItemSize()}px` }
  : {};
...
const focusItem = (side: "left" | "right", key: string): void => {
  const rows = Array.from(host.shadowRoot?.querySelectorAll<HTMLElement>(`[data-transfer-side="${side}"]`) || []);
...
const scrollToItem = (side: "left" | "right", key: string): void => {
  ...
  const top = props.virtual ? index * numericItemSize() : Math.max(0, index * numericItemSize() - numericHeight() / 2);
  if (body) body.scrollTop = top;
  if (side === "left") leftScrollTop.set(top);
  else rightScrollTop.set(top);
  queueMicrotask(() => focusItem(side, String(key)));
};
const onItemKeydown = (side: "left" | "right", item: TransferViewItem, event: KeyboardEvent): void => {
