...
const labelPosition = (): ProgressLabelPosition => {
  return props.labelPosition === "bottom" ||
...
const circleSize = (): string =>
  `${Math.max(40, Number(props.size) || Number(props.width) || 126)}px`;
...
const normalizedLinecap = (): "butt" | "round" | "square" => {
  return props.strokeLinecap === "butt" || props.strokeLinecap === "square"
    ? props.strokeLinecap
    : "round";
};
...
useHostCssVar("--_progress-track", () =>
  props.trackColor ? surfaceColor(props.trackColor, props.trackColor) : "var(--elf-bg-overlay)",
);
