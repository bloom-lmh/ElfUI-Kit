...
const normalizedRowspan = (): number =>
  Math.max(1, Math.floor(Number(props.rowspan) || 1));
...
useHostCssVar(
  "--_descriptions-item-label-align",
  () => normalizeAlign(props.labelAlign) || "left"
);
useHostCssVar(
  "--_descriptions-item-label-width",
  () => cssSize(props.labelWidth, "88px")
);
