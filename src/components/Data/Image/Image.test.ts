...
describe("elf-image", () => {
  ...
  it("provides motion-safe loading and preview transitions", () => {
    ...
    const previewCss = readFileSync("src/components/Data/Image/preview.scss", "utf8");
  ...
  it("opens from Enter, supports preview shortcuts, traps focus, and restores the trigger", async () => {
    ...
    await tick();
    expect((preview.querySelector(".elf-image-preview__image") as HTMLElement).style.transform)
      .toBe("scale(2)");
