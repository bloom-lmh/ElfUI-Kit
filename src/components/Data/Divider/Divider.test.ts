...
describe("elf-divider", () => {
  ...
  it("exposes separator semantics and uses visible slot text as its name", async () => {
    ...
    expect(element.shadowRoot!.querySelector("slot")?.assignedNodes()[0]?.textContent)
      .toBe("Release boundary");
  });
  ...
  it("uses logical borders and protects long divider content", () => {
    const cssText = readFileSync("src/components/Data/Divider/style.scss", "utf8");
