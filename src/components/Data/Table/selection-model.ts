...
export interface ToggleTableRowSelectionOptions
  extends NormalizeTableSelectionOptions {
  row: TableTreeRow;
...
export const normalizeTableSelection = (
  options: NormalizeTableSelectionOptions,
): string[] => {
  const existing = new Set(options.rows.map((row) => row.key));
  const selected = new Set(
    options.keys.map(String).filter((key) => existing.has(key)),
  );
  ...
  const rowsByDepth = [...options.rows].sort(
  ...
  for (const row of rowsByDepth) {
    ...
    const children = options.rows.filter(
      (child) =>
        child.parentKey === row.key && options.isSelectable(child),
    );
    if (
      children.length > 0 &&
...
export const isTableRowIndeterminate = (
  ...
  return (
    selectedCount > 0 &&
    (!selected.has(row.key) || selectedCount < descendants.length)
  );
};

export const toggleTableRowSelection = (
  options: ToggleTableRowSelectionOptions,
): string[] => {
...
export const toggleAllTableSelection = (
  options: ToggleAllTableSelectionOptions,
): string[] => {
  ...
  const shouldClear =
    summary.allSelected ||
    (summary.indeterminate && !options.selectOnIndeterminate);
  const visibleKeys = new Set(options.visibleRows.map((row) => row.key));
  const retainedKeys = options.selectedKeys.filter(
  ...
  return shouldClear
    ...
    : [
        ...retainedKeys,
        ...disabledKeys,
        ...summary.selectableRows.map((row) => row.key),
      ];
};
...
export const createTableRowCollection = (
  ...
  selectedRows: (selectedKeys) =>
    getSelectedTableRows(allRows, selectedKeys),
});
