...
describe("Table sort-filter model", () => {
  it("reads nested values and keeps filter value types distinct", () => {
    ...
    expect(tableFilterSignature([1, "1", true])).not.toBe(
      tableFilterSignature(["1", 1, true]),
    );
  });
  ...
  it("normalizes runtime filter options, allowed values, duplicates and single selection", () => {
    ...
    expect(normalizeTableFilterValues(target, ["admin", "admin", 2, "missing"])).toEqual([
      "admin",
    ]);
  });
  ...
  it("matches OR values within a column and AND conditions across columns", () => {
    ...
    expect(tableRowMatchesFilters(
      { role: "viewer", profile: { active: true } },
      columns,
      values,
    )).toBe(true);
    expect(tableRowMatchesFilters(
      { role: "viewer", profile: { active: false } },
      columns,
      values,
    )).toBe(false);
    expect(tableRowMatchesFilters(
  ...
  it("uses filterMethod safely and treats a thrown matcher as no match", () => {
    const matcher = vi.fn((value: unknown, row: Record<string, unknown>) =>
      Number(row.score) >= Number(value));
  ...
  it("sorts nested values, sortBy paths and sortMethod without mutating source", () => {
    ...
    expect(sortTableRows([byPaths], source, "id", "ascending").map((row) => row.id))
      .toEqual(["10", "1", "2"]);
    ...
    expect(sortTableRows([byMethod], source, "id", "ascending").map((row) => row.id))
      .toEqual(["10", "2", "1"]);
  ...
  it("leaves remote custom sorting untouched and normalizes sort order cycles", () => {
    ...
    expect(normalizeTableSortOrders(column("id"))).toEqual([
      "ascending",
      "descending",
      "",
    ]);
