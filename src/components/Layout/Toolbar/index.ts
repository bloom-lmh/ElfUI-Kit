...
import type { ToolbarCollapsePosition, ToolbarDensity, ToolbarLocation, ToolbarProps, ToolbarSlots } from "./types";

export type { ToolbarCollapsePosition, ToolbarDensity, ToolbarLocation, ToolbarProps, ToolbarSlots } from "./types";
...
const collapsePosition = (): ToolbarCollapsePosition => props.collapsePosition === "start" ? "start" : "end";
const location = (): ToolbarLocation => {
  const value = String(props.location || "top") as ToolbarLocation;
  return ["top", "bottom", "start", "end", "top-start", "top-end", "bottom-start", "bottom-end", "center"].includes(value)
    ? value
    : "top";
};
...
useHostCssVar("--_toolbar-image-opacity", () => String(Math.max(0, Math.min(1, Number(props.imageOpacity)))));
