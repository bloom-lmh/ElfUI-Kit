...
- [x] 基础案例改为 12 个连续 `span=1` 色块与 3 个 `span=4` 色块，对齐参考图的信息密度。
- [x] Grid/Flex 共享结构图使用蓝色色阶、白色内容和稳定大尺寸，不改变布局组件契约。
- [x] 390px 下隐藏 12 列色块的次级 `span` 文案并收紧字号与 padding，避免文字重叠。
- [x] 中文页面标题与说明移除展示用英文组件名；英文页面深度扫描无中文残留。
- [x] 更新页面测试并完成 Material/Midnight、桌面/移动端截图。
