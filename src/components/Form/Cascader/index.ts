...
const toStyleObject = (value: unknown): Record<string, string> => {
    ...
    return Object.fromEntries(
        Object.entries(value as Record<string, string | number>).map(
...
const config = (): CascaderConfig => {
    ...
    const topExpandTrigger = String(
        props.expandTrigger || "",
    ) as CascaderExpandTrigger;
    const propExpandTrigger = String(
        o.expandTrigger || "",
    ) as CascaderExpandTrigger;
    return {
        label: o.label || "label",
        ...
        checkOnClickLeaf:
            props.checkOnClickLeaf !== false && o.checkOnClickLeaf !== false,
        showPrefix: props.showPrefix !== false && o.showPrefix !== false,
        lazyLoad: o.lazyLoad,
    };
};

const fieldNames = (): Required<
    Pick<CascaderFieldNames, "label" | "value" | "children">
> => {
    const c = config();
    return {
        label: c.label,
...
const normalizePathValue = (value: unknown): CascaderPathValue => {
    if (!Array.isArray(value))
        return value == null || value === "" ? [] : [value as CascaderValue];
...
const optionDisabled = (option: RawOption): boolean => {
    ...
    if (typeof disabled === "function")
        return disabled(option as CascaderOption);
...
const optionTreeDepth = (options = rawOptions()): number =>
    options.reduce(
        (depth, option) =>
            Math.max(depth, 1 + optionTreeDepth(optionChildren(option))),
        0,
    );

const resolvedPanelModeState = useComputed(
    (): Exclude<CascaderPanelMode, "auto"> => {
        if (props.panelMode === "tree" || props.panelMode === "columns")
            return props.panelMode;
        const threshold = Math.max(
            1,
            Math.floor(Number(props.treeThreshold) || 3),
        );
        return optionTreeDepth() > threshold ? "tree" : "columns";
    },
);

const resolvedPanelMode = (): Exclude<CascaderPanelMode, "auto"> =>
    resolvedPanelModeState.value;
...
const pathValues = (path: RawOption[]): CascaderPathValue =>
    path.map(optionValue);
...
const findPathByValue = (
    ...
    for (const option of options) {
        ...
        const childPath = findPathByValue(
            value,
            optionChildren(option),
            nextPath,
        );
...
const selectedPaths = (): RawOption[][] =>
    selectedValues.value
        .map((value) => findPathByValues(value))
        .filter((path) => path.length > 0);
...
const isPathAncestor = (
    ancestor: CascaderPathValue,
    child: CascaderPathValue,
): boolean =>
...
const collectCheckedParentPaths = (
    ...
    for (const option of options) {
        ...
        if (
            children.length > 0 &&
            leaves.length > 0 &&
            leaves.every((leaf) => isPathSelected(leaf))
        ) {
            result.push(path);
...
const collapsedTagEntries = (): CascaderTagEntry[] =>
    tagEntries().slice(visibleTagEntries().length);

const collapseTooltipText = (): string =>
    collapsedTagEntries()
...
const modelValueFromPaths = (
    paths: CascaderPathValue[],
): CascaderModelValue => {
    if (shouldEmitPath()) {
...
const isSearchMode = (): boolean =>
    Boolean(props.filterable && query.value.trim());
...
const matchesFilter = (
    node: CascaderNodeSnapshot,
    keyword: string,
): boolean => {
    try {
        return Boolean(
            (
                (props.filterMethod as CascaderFilterMethod | undefined) ??
                defaultFilter
            )(node, keyword),
        );
    } catch {
    ...
    const visit = (
        ...
        options.forEach((option) => {
            ...
            if (
                snapshot &&
                isSearchTarget(path) &&
                matchesFilter(snapshot, keyword)
            )
                result.push(path);
...
const restoreFocusBeforeClose = (): void => {
    ...
    if (!dropdown || !activeElement || !dropdown.contains(activeElement))
        return;
    host.shadowRoot
...
const openDropdown = (): void => {
    ...
    document.dispatchEvent(
        new CustomEvent(CASCADER_OPEN_EVENT, { detail: host }),
    );
...
const collectLeafValuePaths = (path: RawOption[]): CascaderPathValue[] => {
    ...
    const children = optionChildren(current).filter(
        (item) => !optionDisabled(item),
    );
    if (children.length === 0 || optionLeaf(current) || isCheckStrictly())
...
const setMultiplePath = (path: RawOption[]): void => {
    ...
    const allSelected = leaves.every((leaf) =>
        current.some((item) => samePathValue(item, leaf)),
    );
    const next = allSelected
        ? current.filter(
              (item) => !leaves.some((leaf) => samePathValue(item, leaf)),
          )
        : [
              ...current,
              ...leaves.filter(
                  (leaf) => !current.some((item) => samePathValue(item, leaf)),
              ),
          ];
    selectedValues.set(next);
    activePath.set(path);
...
const removeTag = (event: Event): void => {
    ...
    const next = selectedValues
        ...
        .filter(
            (item) =>
                !samePathValue(item, entry.value) &&
                !covered.some((leaf) => samePathValue(item, leaf)),
        );
    selectedValues.set(next);
    emitChange(next);
    emit(
        "remove-tag",
        shouldEmitPath() ? entry.value : entry.value[entry.value.length - 1]!,
    );
};
...
const onOptionPathClick = (path: RawOption[], event?: Event): void => {
    ...
    if (isMultiple()) {
        if (isCheckStrictly()) {
            if (
                config().checkOnClickNode ||
                (!hasChildren && config().checkOnClickLeaf)
            ) {
                toggleStrictPath(path);
            }
            return;
        }
        if (props.checkable || config().checkOnClickNode || !hasChildren)
...
const onOptionHover = (event: Event): void => {
    ...
    if (optionChildren(option).length === 0 && !optionLeaf(option))
        loadLazyChildren(path);
...
const onTreeClick = (event: Event): void => {
    ...
    if (
        hasChildren &&
        !showPrefix() &&
        !target?.closest?.(".option-checkbox")
    ) {
        event.preventDefault();
        event.stopPropagation();
...
const columnOptions = (option: HTMLButtonElement): HTMLButtonElement[] =>
    Array.from(
        option
            .closest(".column")
            ?.querySelectorAll<HTMLButtonElement>(".option:not(:disabled)") ||
            [],
    );
...
const focusFirstChild = (path: RawOption[]): void => {
    queueMicrotask(() => {
        const column =
            host.shadowRoot?.querySelectorAll<HTMLElement>(".column")[
                path.length
            ];
        focusElement(
...
const onTreeKeydown = (
    ...
    if (event.key === "ArrowRight") {
        ...
        if (!isTreePathExpanded(path)) {
            activePath.set(path);
            emitExpand(path);
            if (optionChildren(currentOption).length === 0)
                loadLazyChildren(path);
            return;
        }
        queueMicrotask(() => {
            const next = treeOptionButtons().find((item) => {
                ...
                return (
                    ...
                    isRawPathPrefix(path, childPath)
                );
            });
            focusElement(next);
        });
...
const isActive = (option: RawOption, column: CascaderColumn): boolean =>
    sameValue(
        optionValue(activePath.value[column.level] ?? {}),
        optionValue(option),
    );
...
const showClear = (): boolean =>
    Boolean(props.clearable && hasValue() && !isDisabled());

const columns = (): CascaderColumn[] => {
    const path =
        open.value && activePath.value.length > 0
            ? activePath.value
            : selectedPath();
...
const isRawPathPrefix = (prefix: RawOption[], path: RawOption[]): boolean =>
    ...
    prefix.every((option, index) =>
        sameValue(optionValue(option), optionValue(path[index] ?? {})),
    );
...
const isTreePathExpanded = (path: RawOption[]): boolean => {
    ...
    return Boolean(
        option &&
        !optionLeaf(option) &&
        isRawPathPrefix(path, activePath.value),
    );
};
    ...
    const visit = (
        ...
        for (const option of options) {
            ...
            entries.push({
                ...
                path,
                column: {
                    key: valuePathKey(pathValues(parentPath)),
                    level,
                    parentPath,
                    options,
                },
...
const placement = (): CascaderPlacement =>
    resolvePlacement(popperOptions().placement || props.placement);

const popperModifier = (name: string): CascaderPopperModifier | undefined =>
    popperOptions().modifiers?.find(
        (item) => item.name === name && item.enabled !== false,
    );

const popperOffset = (): [number, number] =>
    popperModifier("offset")?.options?.offset || [0, -1];

const overflowPadding = (): number =>
    Math.max(
        0,
        Number(popperModifier("preventOverflow")?.options?.padding) || 8,
    );
...
const shouldRenderDropdown = (): boolean =>
    Boolean(props.persistent) || open.value;

const getTriggerEl = (): HTMLElement | null =>
    host.shadowRoot?.querySelector<HTMLElement>(".trigger") || null;
...
const updateOverlayPosition = (): void => {
    ...
    const measuredDropdownWidth =
        dropdownRect.width || dropdown.offsetWidth || 184;
    const width = props.fitInputWidth
        ? anchorRect.width
        : measuredDropdownWidth;
    ...
    overlayStyle.set({
        ...
        bottom: "auto",
        margin: "0",
        width: props.fitInputWidth
            ? `${Math.round(width * 100) / 100}px`
            : "auto",
        minWidth: props.fitInputWidth
            ? `${Math.round(anchorRect.width * 100) / 100}px`
            : "0px",
    });
};
...
const syncTopLayer = (): void => {
    const dropdown = getDropdownEl() as
        | (HTMLElement & {
              showPopover?: () => void;
              hidePopover?: () => void;
          })
...
useEventListener<CustomEvent<HTMLElement>>(
    document,
    CASCADER_OPEN_EVENT,
    (event) => {
...
useHostCssVar(
    "--_cascader-item-size",
    () => `${Math.max(28, Number(props.itemSize) || 34)}px`,
);
useHostCssVar(
    "--_cascader-height",
    () => `${Math.max(68, Number(props.height) || 204)}px`,
);
...
const Cascader = defineHtml<CascaderProps, CascaderEmits, CascaderSlots>(`
  <div class="trigger" part="trigger" :tabindex=${props.filterable ? undefined : 0}
    ...
    <input v-if=${props.filterable} class="filter-input" type="text"
      autocomplete="off" :value=${query}
      :placeholder=${hasValue() ? displayLabel() : props.placeholder}
      :aria-label=${placeholderText()} @click=${stopClick} @input=${onFilterInput}
      @keydown=${onFilterKeydown} @focus=${onTriggerFocus} @blur=${onTriggerBlur} />
    <span v-else-if=${!hasValue()} class="placeholder">${placeholderText()}</span>
    <slot v-else-if=${isMultiple() && hasValue()} name="tag"
      :data=${getCheckedNodes()}>
      <span class="tags value">
        <span v-for="entry in visibleTagEntries()" :key="entry.key"
          :class=${tagClass()}>
        ...
        <span v-if="collapsedTagEntries().length > 0"
    ...
    <span class="suffix" part="suffix">
      <button v-if=${showClear()} type="button" class="clear"
        :aria-label=${locale.t("common.clear")} @click=${clear}><span
          v-if=${props.clearIcon}>${props.clearIcon}</span><svg v-else
  ...
  <div v-if=${shouldRenderDropdown() && !isDisabled()} :class=${dropdownClass()}
    :style=${dropdownStyle()} part="dropdown"
    :popover=${props.teleported ? "manual" : undefined}
    :data-append-to=${typeof props.appendTo === "string" ? props.appendTo : "element"}
    role="menu" :aria-hidden=${open ? "false" : "true"}
    :inert=${open ? undefined : ""} @click=${stopClick}
    @keydown=${onDropdownKeydown}>
    <slot name="header"></slot>
    <div v-if=${isSearchMode()} class="filter-results" role="listbox"
      @click=${onFilterResultsClick}>
      <div v-if=${filtering} class="empty" aria-live="polite">
        ${locale.t("table.loading")}</div>
      <template v-else>
        <button v-for="path in filteredPaths"
          :key="valuePathKey(pathValues(path))" class="filter-option"
          type="button" role="option"
          :data-path-key="valuePathKey(pathValues(path))"
          :aria-selected="isPathSelected(pathValues(path)) ? 'true' : 'false'">
          <slot name="suggestion-item" :node="nodeSnapshot(path)"
    ...
    <div v-else-if=${resolvedPanelMode() === "tree"} class="tree-panel" role="tree"
      @click=${onTreeClick}>
      <button v-for="entry in treeEntries()" :key="entry.key"
        :data-path-key="entry.key" :style="treeOptionStyle(entry)" type="button"
        :class="['option', 'tree-option', optionClass(entry.option, entry.column)]"
        :disabled="optionDisabled(entry.option)" role="treeitem"
        ...
        :aria-checked="ariaChecked(entry.option, entry.column)"
        ...
        <span v-if=${showPrefix()} class="option-checkbox"
          :class="checkboxClass(entry.option, entry.column)">
          <span class="checkbox-mark"></span>
        </span>
        <span class="option-label">
          <slot v-if=${hasDefaultOptionSlot()} :node="nodeSnapshot(entry.path)"
            :data="entry.option"></slot>
          <template v-else>{{ optionLabel(entry.option) }}</template>
        </span>
        <span
          v-if="optionLeaf(entry.option) && isSelected(entry.option, entry.column) && !props.checkable"
          class="check">✓</span>
      </button>
    </div>
    <div v-else class="columns" @click=${onColumnsClick}>
      <div v-for="column in columns()" :key="column.key" class="column">
        <button v-for="option in column.options"
          :key="optionKey(option, column.level)"
          :data-path-key="optionPathKey(option, column)" type="button"
          ...
          <span class="option-label">
            <slot v-if=${hasDefaultOptionSlot()}
              :node="nodeSnapshot(optionPath(option, column))" :data="option">
            </slot>
            <template v-else>{{ optionLabel(option) }}</template>
          </span>
          <span v-if="!optionLeaf(option)" class="option-arrow">{{
            isLazyLoading(option) ? '…' : '›' }}</span>
          <span v-else-if="isSelected(option, column) && !props.checkable"
            class="check">✓</span>
        </button>
      </div>
    </div>
