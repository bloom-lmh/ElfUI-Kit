...
export const normalizeMenuItems = (
  ...
  return rawItems.map((raw, position) => {
    ...
    if (group) {
      const index = String(
        item[fields.index] ??
          [...ancestors, `__group_${level}_${position}`].join("/"),
      );
      const label = String(item[fields.group] ?? "");
      const children = normalizeMenuItems(
    ...
    const index = String(
      item[fields.index] ?? [...ancestors, String(position)].join("-"),
    );
    const children = normalizeMenuItems(
      item[fields.children],
      fields,
...
export const hasActiveMenuDescendant = (
  item: MenuViewItem,
  activeIndex: string,
): boolean =>
  item.children.some(
...
export const resolveMenuTrigger = (
  value: unknown,
  compatibilityValue: unknown,
): MenuTrigger =>
  String(value || compatibilityValue || "click") === "hover"
    ? "hover"
    : "click";
...
export const resolveMenuRoutePath = (
  item: Pick<MenuViewItem, "index" | "route">,
): string => {
