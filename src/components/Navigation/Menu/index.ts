...
const isSvgIconPath = (icon: string): boolean => /^M[\d\s,.-]+[A-Za-z]/u.test(icon);
...
const clearSearch = (): void => {
  ...
  shadow?.querySelectorAll<HTMLInputElement>(".menu-search input").forEach((input) => {
    input.value = "";
  });
  searchText.set("");
  host
    .querySelectorAll<HTMLInputElement>('input[slot="search"], [slot="search"] input')
    .forEach((input) => {
      input.value = "";
    });
};
...
onMounted(() => {
  compositionObserver = new MutationObserver(() =>
    compositionVersion.set(compositionVersion.peek() + 1),
  );
  compositionObserver.observe(host, {
    attributes: true,
    childList: true,
    subtree: true,
    characterData: true,
...
const dataItems = useComputed(() => normalizeMenuItems(props.items, fieldNames.value));

const compositionItems = useComputed(() => {
  void compositionVersion.value;
...
const findItem = (index: string): MenuViewItem | undefined => findMenuItem(menuTree.value, index);
...
const activeHorizontalRoot = (): MenuViewItem | undefined => {
  ...
  return roots.find(
    (root) =>
      root.index === activeKey.value ||
      flattenMenuItems(root.children).some((item) => item.index === activeKey.value),
  );
};
...
const selectItem = (item: MenuViewItem) => {
  ...
  if (item.source) {
    const detail: MenuItemClickDetail = {
      ...
      ...(isMenuRoute(item.route) ? { route: item.route } : {}),
    };
...
const itemShowTimeout = (item: MenuViewItem): number =>
  item.showTimeout ?? (Number(props.showTimeout) || 0);

const itemHideTimeout = (item?: MenuViewItem): number =>
  item?.hideTimeout ?? (Number(props.hideTimeout) || 0);
...
const itemTabIndex = (item: MenuViewItem): number => {
  ...
  const focusable = getVisibleItems().filter(
    (entry) => !entry.disabled && !entry.divider && !entry.group,
  );
...
const onKeydown = (e: KeyboardEvent) => {
  ...
  if (e.key === "ArrowRight" && item?.hasChildren) {
    ...
    open(item.index);
    requestAnimationFrame(() => {
      const firstChild = shadow?.querySelector<HTMLElement>(
        `[data-index="${item.children[0]?.index}"]`,
      );
      firstChild?.focus();
    });
    return;
  ...
  const items = Array.from(
    root.querySelectorAll<HTMLElement>(
      ".menu-item:not(.is-disabled):not(.is-divider):not(.is-group)",
    ),
  );
  ...
  const nextIndex =
    e.key === "Home"
      ...
      : e.key === "End"
        ? items.length - 1
        : (idx + (["ArrowDown", "ArrowRight"].includes(e.key) ? 1 : -1) + items.length) %
          items.length;
