...
const titleText = (element: HTMLElement): string => {
  ...
  const source = Array.from(element.children).find(
    (child): child is HTMLElement =>
      child instanceof HTMLElement && child.slot === "title",
  );
...
const compositionChildren = (parent: HTMLElement): HTMLElement[] =>
  Array.from(parent.children).filter(
    (child): child is HTMLElement =>
      ...
      MENU_CHILD_TAGS.has(child.tagName.toLowerCase()),
  );
...
const normalizeCompositionItem = (
  ...
  return {
    ...
    popperStyle: (elementValue(element, "popperStyle") ??
      {}) as MenuPopperStyle,
    ...
    expandCloseIcon: String(
      elementValue(element, "expandCloseIcon") ?? "",
    ),
    expandOpenIcon: String(elementValue(element, "expandOpenIcon") ?? ""),
    collapseCloseIcon: String(
      elementValue(element, "collapseCloseIcon") ?? "",
    ),
    collapseOpenIcon: String(
      elementValue(element, "collapseOpenIcon") ?? "",
    ),
    source: element,
    level,
    indexPath: ancestors,
...
export const normalizeCompositionItems = (
  host: HTMLElement,
): MenuViewItem[] =>
