  ...
  <elf-container class="docs-article">
    <elf-icon-provider :options.prop=${toolbarIconOptions}>
      ...
      <elf-playground :title=${t("collapse")} :code=${collapseCode}>
        <div slot="controls" class="demo-controls"><elf-button size="sm"
            @click=${toggleCollapse}>${t("toggleCollapse")}</elf-button><elf-button size="sm"
            :variant=${collapsePosition.value === "start" ? "contained" : "outlined"}
            @click=${keepStart}>${t("start")}</elf-button><elf-button size="sm"
            :variant=${collapsePosition.value === "end" ? "contained" : "outlined"}
            @click=${keepEnd}>${t("end")}</elf-button></div>
        <div class="toolbar-stage">
      ...
      <elf-playground :title=${t("location")} :code=${locationCode}>
        <div slot="controls" class="demo-controls"><elf-button size="sm" variant="outlined"
            @click=${setTopStart}>${t("topStart")}</elf-button><elf-button size="sm" variant="outlined"
            @click=${setTopEnd}>${t("topEnd")}</elf-button><elf-button size="sm" variant="outlined"
            @click=${setBottomStart}>${t("bottomStart")}</elf-button><elf-button size="sm"
            variant="outlined" @click=${setBottomEnd}>${t("bottomEnd")}</elf-button></div>
      ...
      <elf-playground :title=${t("flexible")} :code=${flexibleCode}>
        <div class="flexible-stage"><elf-toolbar :title=${t("library")} color="#1976d2"
            extension-height="96"><elf-button slot="prepend" circle variant="text" dark
              :aria-label=${t("menu")} @click=${onMenu}><elf-icon name="menu"
                size="18"></elf-icon></elf-button><elf-button slot="append" circle variant="text" dark
              :aria-label=${t("favorite")} @click=${onFavorite}><elf-icon name="favorite"
