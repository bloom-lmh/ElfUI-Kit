...
const t = createDocsTranslator({
    ...
    preview: { zh: "紧凑列表让滚动条案例更贴近日常产品界面。", en: "A compact list keeps this example close to everyday product interfaces." },
    top: { zh: "顶部", en: "Top" },
...
const onCommandScroll = (event: CustomEvent<{ scrollTop: number }>): void => {
    ...
    position.set(top <= 1 ? t("top") : top >= max - 1 ? t("bottom") : `${t("distance")} ${Math.round(top)}px`);
};
