...
describe("FlexPage examples", () => {
  it("通过控制台切换方向和间距，并保持三色子项", async () => {
    ...
    const directionFlex = directionPlayground?.querySelector<HTMLElement & { direction: string }>("elf-flex");
    ...
    directionSelect?.dispatchEvent(new CustomEvent("update:modelValue", { detail: "column-reverse" }));
  ...
  it("通过控制台切换主轴和交叉轴对齐", async () => {
    ...
    const justifyFlex = playgrounds?.[0]?.querySelector<HTMLElement & { justify: string }>("elf-flex");
  ...
  it("通过控制台切换换行、多行对齐和 Space 兼容输入", async () => {
    ...
    expect(page.shadowRoot?.querySelectorAll(".align-content-demo .diagram-item small")).toHaveLength(3);
