...
const propsRows = () => [
  row("items", "MenuItem[]", "[]", "菜单数据，支持 children、divider、group 和 badge。", "Menu data with children, dividers, groups, and badges."),
  row("modelValue", "string", "''", "当前激活项，支持受控绑定。", "Current active item for controlled binding."),
  row("defaultActive", "string", "''", "非受控初始激活项。", "Initial active item in uncontrolled mode."),
  row("defaultOpeneds", "string[]", "[]", "默认展开的子菜单索引。", "Submenu indexes opened initially."),
  row("mode", "vertical | horizontal", "vertical", "菜单布局模式。", "Menu layout mode."),
  row("theme", "light | dark", "light", "菜单主题配色。", "Menu color theme."),
  row("collapse", "boolean", "false", "折叠垂直菜单为图标栏。", "Collapse a vertical menu into an icon rail."),
  row("uniqueOpened", "boolean", "false", "只保持一个展开分支。", "Keep only one branch open."),
  row("router", "boolean", "false", "将路径索引同步到 location.hash。", "Sync path indexes to location.hash."),
  row("props", "MenuFieldNames", "built-in", "自定义数据字段映射。", "Customize item field mappings."),
  ...
  row("indent", "number", "20", "每级子菜单的缩进像素。", "Indent in pixels for each submenu level."),
  ...
  row("togglePlacement", "footer | header", "header", "折叠按钮所在区域。", "Region containing the collapse control."),
  row("searchable", "boolean", "false", "显示菜单搜索框。", "Show the menu search field."),
  row("searchPlaceholder", "string", "localized", "搜索输入占位文本。", "Search input placeholder."),
  row("ellipsis", "boolean", "false", "在水平菜单中显示更多入口。", "Show a more entry in horizontal mode."),
  ...
  row("showTimeout / hideTimeout", "number", "0 / 300", "悬停展开与关闭延迟。", "Hover open and close delays."),
  row("popperOffset", "number", "4", "子菜单浮层偏移距离。", "Submenu overlay offset."),
  row("popperClass / popperStyle", "string / object", "-", "扩展浮层类名与行内样式。", "Extend overlay classes and inline styles."),
  ...
  row("closeOnClickOutside", "boolean", "true", "点击水平浮层外部时关闭。", "Close a horizontal overlay on outside click."),
  row("persistent", "boolean", "true", "关闭后保留浮层 DOM。", "Keep overlay DOM after closing."),
];

const eventsRows = () => [
  row("update:modelValue", "(index: string) => void", "-", "提交当前激活项。", "Commit the active item."),
  row("select", "(index, indexPath, item) => void", "-", "选择叶子菜单项。", "A leaf menu item was selected."),
  row("open", "(index, indexPath, item) => void", "-", "展开子菜单分支。", "A submenu branch was opened."),
  row("close", "(index, indexPath, item) => void", "-", "关闭子菜单分支。", "A submenu branch was closed."),
  row("collapse-change", "(collapsed: boolean) => void", "-", "折叠状态变化。", "Collapse state changed."),
];
...
const methodsRows = () => [
  ...
  row("updateActiveIndex(index)", "Function", "-", "同步当前激活索引。", "Synchronize the active index."),
];

const subMenuRows = () => [
  row("index", "string", "-", "子菜单唯一标识，必填。", "Required unique submenu identifier."),
  row("title / icon / badge", "string", "-", "子菜单标题、图标和徽章。", "Submenu title, icon, and badge."),
  row("disabled", "boolean", "false", "禁用整个子菜单。", "Disable the entire submenu."),
  row("popperClass / popperStyle", "string / object", "-", "当前子菜单的浮层样式。", "Overlay styling for this submenu."),
  row("showTimeout / hideTimeout", "number", "inherit Menu", "当前子菜单的展开与关闭延迟。", "Open and close delays for this submenu."),
  row("popperOffset", "number", "inherit Menu", "当前子菜单的浮层偏移。", "Overlay offset for this submenu."),
  row("teleported", "boolean", "by level", "浮层传送语义兼容属性。", "Compatibility flag for overlay teleport semantics."),
  row("expand*Icon / collapse*Icon", "string", "-", "展开与折叠状态图标。", "Expanded and collapsed state icons."),
  row("title", "slot", "-", "自定义子菜单标题。", "Custom submenu title."),
];

const menuItemRows = () => [
  row("index", "string", "-", "菜单项唯一标识，必填。", "Required unique menu item identifier."),
  row("title / icon / badge", "string", "-", "菜单项标题、图标和徽章。", "Menu item title, icon, and badge."),
...
const menuItemGroupRows = () => [
  row("title", "string", "-", "分组标题，也可由 title 插槽提供。", "Group title, also available through the title slot."),
  row("default", "slot", "-", "分组内的 MenuItem。", "MenuItem children in the group."),
];

const slotsRows = () => [
  row("default", "slot", "-", "组合式 MenuItem、SubMenu 和 MenuItemGroup。", "Compositional MenuItem, SubMenu, and MenuItemGroup content."),
