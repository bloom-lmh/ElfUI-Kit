...
beforeAll(async () => {
  ...
  const [{ PageCalendar }, { PageCalendarEx2 }, { PageCalendarEx4 }] =
    await Promise.all([import("./index"), import("./ex2"), import("./ex4")]);
...
describe("Calendar documentation", () => {
  ...
  it("demonstrates the Provider week-start fallback without a local override", async () => {
    ...
    const provider = page.shadowRoot?.querySelector(
      "elf-config-provider",
    ) as HTMLElement & { config?: { date?: { firstDayOfWeek?: number } } };
    const calendar = page.shadowRoot?.querySelector(
      "elf-calendar",
    ) as HTMLElement & { firstDayOfWeek?: number };
    expect(provider.config?.date?.firstDayOfWeek).toBe(0);
    expect(calendar.firstDayOfWeek).toBeUndefined();
    expect(
      calendar.shadowRoot?.querySelectorAll<HTMLElement>(".week span")[1]
        ?.textContent,
    ).toContain("日");
  });
  ...
  it("keeps the keyboard example constrained to a compact viewport", async () => {
    ...
    expect(
      page.shadowRoot?.querySelector(".demo-calendar.is-compact"),
    ).not.toBeNull();
    expect(
      (
