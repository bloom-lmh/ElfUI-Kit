...
const t = createDocsTranslator({
  ...
  keyboard: { zh: "使用 ↑ ↓、Home、End 移动焦点，Enter 或 Space 选择", en: "Use ↑ ↓, Home, End to move; Enter or Space to select" },
...
const channels = (): ChannelItem[] => [
  { id: "design", icon: "◇", title: t("design"), subtitle: t("designSub") },
  { id: "engineering", icon: "</>", title: t("engineering"), subtitle: t("engineeringSub") },
  { id: "archive", icon: "□", title: t("archive"), subtitle: t("archiveSub"), disabled: true },
  { id: "support", icon: "◎", title: t("support"), subtitle: t("supportSub") }
];
...
// Derived state
const selectedLabel = (): string => channels().find((item) => item.id === selected.value)?.title || "—";

// Methods
const onSelect = (event: CustomEvent<string>): void => selected.set(String(event.detail));
const clear = (): void => selected.set("");

const code = `<elf-list aria-label="工作频道" bordered>
  <elf-list-item
...
const script = `const selected = useRef("design");
    ...
    <section class="list-selection-card">
      ...
      <elf-list :ariaLabel.prop=${t("region")} bordered>
        <elf-list-item
          ...
          @select=${onSelect}
        >
          <span slot="leading" class="list-channel-icon">{{ channel.icon }}</span>
