...
const t = createDocsTranslator({
  ...
  title: { zh: "标题快捷文本，title 插槽优先", en: "Title shortcut; the title slot takes precedence" },
  extra: { zh: "头部尾端快捷文本，extra 插槽优先", en: "Trailing header shortcut; the extra slot takes precedence" },
  ...
  fieldProps: { zh: "映射 key、label、value、span 字段", en: "Maps key, label, value, and span fields" },
  defaultSlot: { zh: "声明式 DescriptionsItem 子项", en: "Declarative DescriptionsItem children" },
  titleSlot: { zh: "自定义标题，可独立于 title prop 使用", en: "Custom title, usable without the title prop" },
  extraSlot: { zh: "自定义头部操作，可独立于 extra prop 使用", en: "Custom header action, usable without the extra prop" },
  emptySlot: { zh: "没有数据项或声明式子项时的空状态", en: "Empty state when no data or declarative items exist" },
  label: { zh: "标签快捷文本，label 插槽优先", en: "Label shortcut; the label slot takes precedence" },
  span: { zh: "横向跨列数，会按当前响应式列数收敛", en: "Column span, clamped to the responsive column count" },
  ...
  itemEmptyText: { zh: "未提供默认插槽时的占位文本", en: "Fallback when the default slot is not provided" },
  contentSlot: { zh: "支持文本、链接、Tag 等丰富值内容", en: "Rich value content such as text, links, or tags" },
...
const propsRows = () => [
  ...
  { name: "props", type: "DescriptionsFieldNames", default: "default fields", desc: t("fieldProps") }
];
