...
describe("Card documentation", () => {
  ...
  it("finishes loading and recovers a failed shortcut cover", async () => {
    ...
    const loadingToggle = page.shadowRoot!.querySelector<HTMLButtonElement>(".card-demo-actions button")!;
    loadingToggle.click();
    await tick();
    expect(reportCard.hasAttribute("loading")).toBe(false);

    mediaCard.shadowRoot!.querySelector<HTMLImageElement>(".card-image-wrap img")!
