...
const createItems = (start: number, count: number): ActivityItem[] => Array.from({ length: count }, (_, offset) => {
  ...
  const detail = id % 4 === 0
    ...
    : id % 3 === 0
      ? pick("已完成接口联调，等待设计与无障碍复核。", "Integration is complete; design and accessibility reviews are pending.")
      : pick("状态正常。", "Status is healthy.");
  return { id, title: pick(`动态记录 #${String(id).padStart(3, "0")}`, `Activity #${String(id).padStart(3, "0")}`), detail };
});
...
const appendItems = (): void => {
  ...
  setTimeout(() => {
    ...
    loading.set(false);
    activity.set(pick(`${items.value.length} 条 · 已保持滚动锚点`, `${items.value.length} items · scroll anchor preserved`));
  }, 420);
};
...
const toggleEmpty = (): void => {
  ...
  activity.set(items.value.length > 0 ? pick("已恢复 240 条", "Restored 240 items") : pick("空状态", "Empty state"));
};
