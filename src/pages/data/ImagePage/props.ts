...
const t = createDocsTranslator({
  ...
  alt: { zh: "图片替代文本；装饰图片应传空字符串", en: "Alternative text; use an empty string for decorative images" },
  fit: { zh: "对应原生 object-fit 的填充方式", en: "Fitting mode mapped to native object-fit" },
  size: { zh: "数字和纯数字字符串按 px 处理，也支持 CSS 尺寸", en: "Numbers and numeric strings use px; CSS sizes are also accepted" },
  lazy: { zh: "接近视口前不创建带 src/srcset 的 img", en: "Delay an img with src/srcset until it nears the viewport" },
  list: { zh: "非空时启用点击与键盘预览", en: "Enable pointer and keyboard preview when non-empty" },
  initial: { zh: "每次打开预览时使用的初始索引", en: "Initial index used whenever the preview opens" },
  teleported: { zh: "将预览层传送到 document.body", en: "Teleport the preview layer to document.body" },
  ...
  errorSlot: { zh: "失败内容；未提供时显示内置重试按钮", en: "Failure content; the fallback includes a retry button" },
  loadingSlot: { zh: "延迟请求或加载中的占位内容", en: "Placeholder shown before request or while loading" },
  openExpose: { zh: "以 initial-index 打开预览", en: "Open the preview at initial-index" },
  closeExpose: { zh: "关闭预览并恢复触发器焦点", en: "Close the preview and restore trigger focus" },
...
const propsRows = () => [
  ...
  { name: "fit", type: "fill | contain | cover | none | scale-down", default: "fill", desc: t("fit") },
