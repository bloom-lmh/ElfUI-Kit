...
import type { InfiniteScrollExposes } from "../../../components/Data/InfiniteScroll";
...
const t = createDocsTranslator({
  ...
  hintExternal: { zh: "监听祖先容器，不创建第二层滚动", en: "Listen to an ancestor without nesting another scroller" },
  hintWindow: { zh: "继续滚动页面，在组件底部附近加载", en: "Keep scrolling the page to load near the document bottom" },
...
const hint = (): string => t(`hint${mode.value[0]!.toUpperCase()}${mode.value.slice(1)}`);
...
const onModeAction = (event: Event): void => {
  const value = event.composedPath()
    .find((entry): entry is HTMLElement =>
      entry instanceof HTMLElement && Boolean(entry.dataset.mode)
    )
