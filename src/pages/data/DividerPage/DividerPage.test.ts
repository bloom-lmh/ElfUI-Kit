...
describe("Divider documentation", () => {
  it("combines line styles and text positions into one comparison matrix", async () => {
    ...
    expect(Array.from(dividers, (divider) => divider.getAttribute("border-style")))
      .toEqual(["solid", "dashed", "dotted", "double"]);
    expect(Array.from(dividers, (divider) => divider.getAttribute("content-position")))
      .toEqual(["left", "center", "right", "center"]);
    expect(Array.from(dividers).every((divider) => divider.getAttribute("role") === "separator"))
      .toBe(true);
  });
