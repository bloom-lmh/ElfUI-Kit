...
const rows = [
  { name: "label / background-color", type: "string", default: "''", desc: p("浮动标签与字段背景主题", "Floating label and field background styling.") },
  ...
  { name: "variant", type: "FieldVariant", default: "outlined", desc: p("字段表面样式", "Field surface variant.") }
];
...
const PageInputTagProps = defineHtml(`
  ...
  <elf-props-table title="Events" :rows=${[
    { name: "update:modelValue / add-tag / remove-tag / clear", type: "CustomEvent", desc: p("标签数组变化时触发", "Emitted as the tag collection changes.") }
  ]} />
