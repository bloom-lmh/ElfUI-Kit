...
describe("TreeSelectPage", () => {
  it("provides complete Template and Script sections for every capability", async () => {
    ...
    expect(
      playgrounds.every((playground) =>
        Boolean(playground.code?.includes("elf-tree-select")),
      ),
    ).toBe(true);
    expect(
      playgrounds.every((playground) => Boolean(playground.script?.trim())),
    ).toBe(true);
  ...
  it("renders only a virtual Tree window for the 10,000-node example", async () => {
    ...
    await wait(40);
    const tree =
      virtualSelect.shadowRoot!.querySelector<HTMLElement>("elf-tree")!;

    expect(tree.shadowRoot!.querySelectorAll(".tree-node").length).toBeLessThan(
      30,
    );
    expect(
      (tree.shadowRoot!.querySelector(".tree-window") as HTMLElement).style
        .paddingBottom,
    ).not.toBe("0px");
  });
  ...
  it("keeps page copy, playground titles, runtime labels, and API descriptions bilingual", async () => {
    ...
    const playgrounds = components.flatMap((component) =>
      Array.from(
        component.shadowRoot!.querySelectorAll<HTMLElement>("elf-playground"),
      ),
    );
    expect(
      playgrounds.map((playground) => playground.getAttribute("title")),
    ).toEqual([
      "Basic selection",
    ...
    expect(
      deepQuery(page.shadowRoot!, "elf-tree-select")?.textContent,
    ).not.toContain("选择团队");
    const api = components.find((component) =>
      component.shadowRoot?.querySelector("elf-props-table"),
    )!;
    const table = api.shadowRoot?.querySelector<
      HTMLElement & { rows?: Array<{ desc?: string }> }
    >("elf-props-table");
    expect(
      table?.rows?.some(
        (row) => row.desc === "Reuse Tree's virtual window for large data.",
      ),
    ).toBe(true);
  });
});
