...
describe("CascaderPage", () => {
  ...
  it("英文模式覆盖页面、九个案例、运行数据和 API", async () => {
    const provider = document.createElement("elf-locale-provider") as HTMLElement & { name?: string };
    provider.name = "en-US";
    ...
    const components = Array.from(page.shadowRoot?.querySelectorAll<HTMLElement>("*") ?? [])
    ...
    const formItem = deepQuery<HTMLElement & { label?: string }>(deepExample.shadowRoot!, "elf-form-item");
    expect(formItem?.label).toBe("Release catalog");

    const api = components.find((component) => component.shadowRoot?.querySelector("elf-props-table"))!;
    const propsTable = api.shadowRoot?.querySelector<HTMLElement & { rows?: Array<{ desc?: string }> }>(
      "elf-props-table",
    );
    expect(propsTable?.rows?.some((row) => row.desc === "Automatically use a compact tree for deep paths"))
      .toBe(true);
  });
});
