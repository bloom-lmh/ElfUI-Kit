...
const t = createDocsTranslator({
  ...
  failure: { zh: "服务暂时不可用，请修改关键字后重试", en: "The service is unavailable. Change the keyword and retry." },
...
const emptyMessage = (): string => remoteError.value
  ? remoteError.value
  : keyword.value ? t("noMatch") : t("start");
...
const remoteMethod = async (query: string): Promise<void> => {
  ...
  remoteOptions.set(directory().filter((item) =>
