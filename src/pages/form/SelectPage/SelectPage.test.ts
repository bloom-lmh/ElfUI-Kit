...
describe("SelectPage", () => {
  ...
  it("大数据案例只渲染虚拟窗口", async () => {
    ...
    expect(select.shadowRoot?.querySelector<HTMLElement>(".options-track")?.style.height)
      .toBe("400000px");
  });
  ...
  it("英文模式覆盖页面、案例目录、运行数据和 API", async () => {
    ...
    const props = components.find((component) => component.shadowRoot?.querySelector("elf-props-table"))!;
    const propsTable = props.shadowRoot?.querySelector<HTMLElement & { rows?: Array<{ desc?: string }> }>(
      "elf-props-table",
    );
    expect(propsTable?.rows?.some((row) => row.desc === "Enable virtualization and its minimum item count."))
      .toBe(true);
  });
});
