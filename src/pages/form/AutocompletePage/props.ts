...
const rows = [
  { name: "variant / label", type: "FieldVariant / string", default: "filled / ''", desc: pick("六种统一字段表面与浮动标签", "Six unified field surfaces and a floating label") },
  ...
  { name: "allow-create / create-text", type: "boolean / string", default: "false / 'Create'", desc: pick("输入不存在时提供创建项", "Offer a create option for unmatched input") },
  { name: "virtual / item-height / max-height / overscan", type: "boolean / number", default: "false / 40 / 280 / 3", desc: pick("固定高度长列表虚拟滚动", "Virtualize fixed-height long lists") },
  { name: "loading / loading-text", type: "boolean / string", default: "false / 'Loading...'" },
  { name: "no-data-text / error-text", type: "string", default: "No suggestions / Unable to load suggestions", desc: pick("远程空态与错误态文案", "Remote empty and error messages") },
...
const PageAutocompleteProps = defineHtml(`
  ...
  <elf-props-table title="Events" :rows=${[
    ...
    { name: "create", type: "(option: AutocompleteOption) => void", desc: pick("选择创建项时触发", "Emitted when the create option is selected") },
    { name: "focus / blur / clear", type: "FocusEvent / void" },
    { name: "fetch-error", type: "(error: unknown) => void", desc: pick("远程建议请求失败", "Remote suggestion request failed") }
  ]} />
  <elf-props-table title="Slots" :rows=${[
    { name: "default", desc: pick("自定义建议内容，接收 item", "Custom suggestion content; receives item") },
    { name: "loading", desc: pick("远程加载内容", "Remote loading content") },
    { name: "empty", desc: pick("远程或本地无匹配结果", "Remote or local empty results") },
    { name: "error", desc: pick("远程请求失败内容，接收 error", "Remote request failure content; receives error") }
  ]} />
