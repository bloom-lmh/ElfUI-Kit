...
const propsRows = [
  ...
  { name: "density", type: "default | comfortable | compact", default: "default", desc: pick("Material 字段密度；非默认 density 覆盖兼容 size 高度", "Material field density; non-default density overrides compatibility size height") },
  { name: "variant", type: "default | filled | outlined | underlined | solo | solo-filled | solo-inverted", default: "filled", desc: pick("字段表面样式；filled 是 default 的兼容别名", "Field surface; filled is a compatibility alias for default") },
  { name: "placeholder", type: "string" },
  { name: "disabled", type: "boolean" },
  { name: "readonly", type: "boolean" },
  { name: "clearable", type: "boolean", desc: pick("可清空", "Clearable") },
  { name: "clear-icon", type: "string", default: "''", desc: pick("自定义清空图标文本；默认使用线性 SVG 图标", "Custom clear icon text; defaults to a line SVG icon") },
  { name: "show-password", type: "boolean", desc: pick("密码可见切换", "Password visibility toggle") },
  { name: "formatter", type: "(value: string) => string", desc: pick("展示值格式化", "Display value formatter") },
  { name: "parser", type: "(value: string) => string", desc: pick("输入值反解析", "Input value parser") },
  ...
  { name: "min / max / step", type: "string | number", desc: pick("透传给原生 input", "Forwarded to the native input") },
  ...
  { name: "background-color", type: "string", default: "''", desc: pick("自定义字段背景色，悬浮与聚焦时保持一致", "Custom field background preserved on hover and focus") },
  { name: "label", type: "string", desc: pick("浮动标签，同时作为无 aria-label 时的可访问名称兜底", "Floating label and accessible-name fallback without aria-label") },
...
const slotsRows = [
  ...
  { name: "password-icon", desc: pick("自定义密码可见性切换内容", "Custom password visibility content") }
];
...
const exposesRows = [
  ...
  { name: "passwordVisible", type: "Ref<boolean>" },
  {
    ...
    desc: pick("兼容 Element Plus 暴露项；多行输入请使用 Textarea 组件", "Element Plus compatibility expose; use Textarea for multiline input")
  }
];
