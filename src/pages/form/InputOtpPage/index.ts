...
const PageInputOtp = defineHtml(`
  <elf-container>
    <h1>${p("一次性密码", "One-time password input")}</h1>
    <p>${p("用于验证码和支付密码等分段输入场景，支持长度、数字模式、分隔符、只读与禁用状态。", "Use segmented input for verification or payment codes with configurable length, numeric mode, separators, and read-only or disabled states.")}</p>
