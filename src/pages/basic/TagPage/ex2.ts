...
const selectedLabels = (): string => {
  const labels = FILTER_OPTIONS.filter((option) => isSelected(option.id)).map((option) => option.label);
...
const activityText = (): string => `${t("count")} ${tagItems.value.length} · ${t(activity.value as "ready" | "added" | "renamed" | "removed")}`;
