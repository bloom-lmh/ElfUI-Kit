...
const t = createDocsTranslator({
  ...
  type: { zh: "Element Plus 兼容语义色，优先于 color", en: "Element Plus-compatible semantic color; takes precedence over color" },
  ...
  checked: { zh: "启用可选择模式并控制选中状态", en: "Enables selectable mode and controls pressed state" },
  ...
  close: { zh: "关闭按钮激活时触发；组件不会自行删除", en: "Emitted when the close button activates; the component does not remove itself" },
  change: { zh: "可选择状态变化时触发，detail 为 boolean", en: "Emitted when selectable state changes; detail is boolean" },
  update: { zh: "checked 双向绑定事件", en: "Two-way binding event for checked" },
  content: { zh: "标签内容；超长内容在受限宽度内省略", en: "Tag content; long content truncates in constrained widths" }
});

const propsRows = () => [
  { name: "type", type: "primary | secondary | success | warning | danger | info", default: "''", desc: t("type") },
