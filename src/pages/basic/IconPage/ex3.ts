...
const t = createDocsTranslator({
  ...
  semantic: { zh: "独立图标自身承载语义时使用 aria-label", en: "Use aria-label when a standalone icon carries meaning" },
  keyboard: { zh: "按钮可通过 Tab 聚焦并使用 Enter 或 Space", en: "Use Tab to focus the button, then Enter or Space to activate it" }
});
...
<elf-icon name="●" color="var(--elf-success)" aria-label="Service available"></elf-icon>
<elf-icon loading name="◌" aria-label="Syncing"></elf-icon>`;
...
const PageIconEx3 = defineHtml(`
  <elf-playground :title=${t("title")} :code=${accessibilityCode} :script=${accessibilityScript}>
    ...
    <div class="icon-accessibility-panel">
      <div class="icon-demo-row">
        <elf-button circle :aria-label=${t("refresh")}><elf-icon name="↻"></elf-icon></elf-button>
        <elf-button shape="square" variant="outlined" :aria-label=${t("settings")}>
          <elf-icon name="⚙"></elf-icon>
        </elf-button>
        <elf-icon loading name="◌" size="28" color="var(--elf-primary)" :aria-label=${t("syncing")}></elf-icon>
      </div>
      <div>
