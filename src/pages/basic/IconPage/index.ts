...
const t = createDocsTranslator({
  ...
  fallback: { zh: "已知图标集中找不到名称时的回退内容", en: "Fallback when a name is missing from a known set" },
  size: { zh: "数字按 px 处理，也支持任意 CSS 尺寸", en: "Numbers use px; any CSS size is also accepted" },
  color: { zh: "图标颜色，默认继承 currentColor", en: "Icon color; inherits currentColor by default" },
  aria: { zh: "图标自身承载语义时的无障碍名称", en: "Accessible name when the icon itself carries meaning" },
  loading: { zh: "启用旋转；遵循减少动态效果偏好", en: "Enables rotation and respects reduced-motion preferences" },
  options: { zh: "defaultSet、aliases 与 sets 的组合配置", en: "Combined defaultSet, aliases, and sets configuration" },
  ...
  sets: { zh: "通过 createSvgIconSet/createClassIconSet 创建的图标集", en: "Sets created with createSvgIconSet/createClassIconSet" },
  inherit: { zh: "继承父 Provider 或全局 configureIcons 配置", en: "Inherits the parent provider or global configureIcons configuration" },
