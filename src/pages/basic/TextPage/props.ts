...
const t = createDocsTranslator({
  ...
  size: { zh: "文字尺寸；同时支持 sm / md / lg 别名", en: "Text size; sm / md / lg aliases are also supported" },
  truncated: { zh: "单行省略；容器需要提供可计算宽度", en: "Single-line ellipsis; the container must provide a computable width" },
  lineClamp: { zh: "最多显示行数，非法值与小于 1 的值归一为 1", en: "Maximum visible lines; invalid values and values below 1 normalize to 1" },
  tag: { zh: "Shadow DOM 内渲染的安全原生语义标签", en: "Safe native semantic element rendered inside the Shadow DOM" },
...
const propsRows = () => [
  { name: "type", type: "'' | primary | success | warning | danger | info", default: "''", desc: t("type") },
  { name: "size", type: "'' | small | default | large | sm | md | lg", default: "''", desc: t("size") },
