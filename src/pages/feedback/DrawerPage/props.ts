...
const propsRows = [
  ...
  { name: "direction", type: "rtl | ltr | ttb | btt", default: "rtl", desc: pick("面板滑入方向。", "Panel slide-in direction.") },
  { name: "size", type: "string", default: "'30%'", desc: pick("水平抽屉的宽度或垂直抽屉的高度。", "Width for horizontal drawers or height for vertical drawers.") },
  { name: "resizable", type: "boolean", default: "false", desc: pick("允许通过内侧边缘或键盘调整尺寸。", "Allow resizing from the inner edge or keyboard.") },
  { name: "min-size", type: "number | string", default: "160", desc: pick("可调整的最小尺寸。", "Minimum resizable size.") },
  { name: "max-size", type: "number | string", default: "'90%'", desc: pick("可调整的最大尺寸。", "Maximum resizable size.") },
  { name: "modal", type: "boolean", default: "true", desc: pick("显示遮罩并声明模态语义。", "Show the backdrop and expose modal semantics.") },
  { name: "close-on-mask", type: "boolean", default: "true", desc: pick("点击遮罩时请求关闭。", "Request closing when the backdrop is clicked.") },
  { name: "close-on-escape", type: "boolean", default: "true", desc: pick("按 Escape 时请求关闭最上层抽屉。", "Request closing the topmost drawer on Escape.") },
...
const eventsRows = [
  ...
  { name: "open-auto-focus", type: "() => void", desc: pick("初始焦点进入抽屉后触发。", "Emitted after initial focus enters the drawer.") },
  { name: "close-auto-focus", type: "() => void", desc: pick("焦点恢复到触发元素后触发。", "Emitted after focus returns to the trigger.") },
  { name: "resize-start", type: "(detail: DrawerResizeDetail) => void", desc: pick("开始拖动或键盘调整时触发。", "Emitted when pointer or keyboard resizing starts.") },
  { name: "resize", type: "(detail: DrawerResizeDetail) => void", desc: pick("尺寸变化时触发。", "Emitted while the size changes.") },
  { name: "resize-end", type: "(detail: DrawerResizeDetail) => void", desc: pick("尺寸调整结束时触发。", "Emitted when resizing ends.") },
];
...
const methodsRows = [
  ...
  { name: "resetSize()", type: "() => void", desc: pick("清除用户调整结果并恢复 size。", "Clear the user-resized value and restore size.") },
];
