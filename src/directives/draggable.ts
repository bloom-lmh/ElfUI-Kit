...
const optionsOf = (element: HTMLElement): DraggableOptions =>
  states.get(element)?.value ?? {};
...
const mount = (
  element: HTMLElement,
  binding: DirectiveBinding<DraggableDirectiveValue>
): void => {
  const listeners = {
    dragstart: (event: DragEvent): void => {
