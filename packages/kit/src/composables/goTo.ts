...
export type GoToEasing = GoToEasingName | ((progress: number) => number);
...
export type GoTo = (target: ScrollTarget, options?: GoToOptions) => GoToTask;
...
const resolveEasing = (easing: GoToEasing | undefined): ((progress: number) => number) =>
...
const targetPosition = (
  ...
  return maximum > 0 ? Math.min(maximum, Math.max(0, position)) : Math.max(0, position);
};
