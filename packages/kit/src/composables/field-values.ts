...
export const DEFAULT_EMPTY_VALUES: readonly unknown[] = Object.freeze([undefined, null, ""]);
...
export interface FieldValueDefaultsResolver {
  ...
  valueOnClear: <T>(local: T | (() => T) | undefined, fallback: () => T) => T;
}
...
export const useFieldValueDefaults = (): FieldValueDefaultsResolver => {
  ...
  return {
    emptyValues,
    isEmpty: (value, local) => emptyValues(local).some((candidate) => Object.is(candidate, value)),
    valueOnClear: <T>(local: T | (() => T) | undefined, fallback: () => T): T => {
      const configured = local !== undefined ? local : provider.config.field?.valueOnClear;
