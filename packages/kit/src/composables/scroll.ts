...
export type ScrollContainerTarget =
  string | ScrollContainer | (() => ScrollContainer | null) | null;
...
export const isWindowContainer = (container: ScrollContainer): container is Window =>
...
export const resolveScrollContainer = (
  ...
  if (typeof target === "string" && target) {
    return query<HTMLElement>(root, target) ?? query<HTMLElement>(document, target) ?? window;
  }
  if (target && typeof target === "object" && "addEventListener" in target) {
    return target;
  }
  return window;
};
...
export const getScrollPosition = (container: ScrollContainer, axis: ScrollAxis = "y"): number => {
...
export const getElementScrollPosition = (
  ...
  return current + (axis === "x" ? rect.left - containerRect.left : rect.top - containerRect.top);
};
...
export const setScrollPosition = (
  ...
  const options: ScrollToOptions =
    axis === "x" ? { left: position, behavior: "auto" } : { top: position, behavior: "auto" };
  if (isWindowContainer(container)) {
