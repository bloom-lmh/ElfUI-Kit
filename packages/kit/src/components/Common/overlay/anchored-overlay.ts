...
const rawPosition = (
  ...
  if (placement === "left" || placement === "right") {
    return {
      left: placement === "left" ? anchor.left - overlay.width - mainAxis : anchor.right + mainAxis,
  ...
  return {
    left: left + crossAxis,
    top: isTop ? anchor.top - overlay.height - mainAxis : anchor.bottom + mainAxis,
  };
};
...
const overflowScore = (
  ...
  return (
    Math.max(0, minLeft - position.left) +
...
export const computeAnchoredPosition = <TPlacement extends AnchoredPlacement>(
  ...
  return {
    left: clamp(
      ...
      viewportLeft + viewport.width - overlay.width - padding,
    ),
    top: clamp(
      position.top,
      viewportTop + padding,
      viewportTop + viewport.height - overlay.height - padding,
    ),
    placement,
  };
};
...
export const isEventInside = (
  ...
  return Array.from(containers).some((container) =>
    Boolean(
      container &&
      path.some(
        (target) => target === container || (target instanceof Node && container.contains(target)),
      ),
    ),
  );
};
...
export const connectAnchoredOverlayLifecycle = (
  ...
  const resizeTargets = Array.from(options.resizeTargets).filter((target): target is Element =>
    Boolean(target),
  );
  const observer =
    resizeTargets.length > 0 && typeof ResizeObserver !== "undefined"
