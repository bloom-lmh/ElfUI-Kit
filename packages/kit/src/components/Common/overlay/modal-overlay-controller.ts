import { createFocusScope, type FocusScopeController } from "../focus/focus-scope";
import { overlayStack, type OverlayStack } from "./overlay-stack";
import {
  createOverlayInteractionController,
  type OverlayInteractionController,
} from "./overlay-interaction-controller";
import type { OverlayCloseReason, OverlayLifecycleState } from "./overlay-protocol";
...
export const createModalOverlayController = (
  ...
  const interaction: OverlayInteractionController = createOverlayInteractionController({
    kind: options.kind,
  ...
  return {
    ...
    trapFocus: (event) => interaction.isActive() && interaction.isTopmost() && focus.trap(event),
