...
const defaultTimeFor = (target: "start" | "end"): string => {
  ...
  const resolved = Array.isArray(value) ? value[target === "end" ? 1 : 0] || value[0] : value;
  return String(resolved || "00:00:00");
};

const parsePart = (value: unknown, target: "start" | "end"): { date: string; time: string } => {
...
const compose = (date: string, time: string, target: "start" | "end"): string => {
  if (!date) return "";
  const parsed = dateService.adapter.parse(`${date}T${time || defaultTimeFor(target)}`);
  return parsed
...
const dateShortcuts = (): Array<{
  ...
  (Array.isArray(props.shortcuts) ? props.shortcuts : []).map((shortcut: DateTimeShortcut) => ({
...
const timeMin = (): string =>
  props.min && startDate.value === dateMin() ? parsePart(props.min, "start").time : "";
...
const clear = (): void => {
  ...
  const value = fieldValues.valueOnClear<DateTimePickerValue>(props.valueOnClear, () =>
    rangeMode() ? ["", ""] : "",
  );
  startDate.set("");
  endDate.set("");
...
const onFocus = (event: CustomEvent<unknown>): void => form.dispatchFocus(focusDetail(event));
const onBlur = (event: CustomEvent<unknown>): void => form.dispatchBlur(focusDetail(event));
