...
describe("elf-time-select", () => {
  it("renders fixed-step options and the controlled value through Select", async () => {
    ...
    expect((innerSelect(element) as HTMLElement & { modelValue: unknown }).modelValue).toBe(
      "09:30",
    );
  ...
  it("emits controlled updates and semantic changes exactly once", async () => {
    ...
    selectTrigger(element).click();
    await tick();
    innerSelect(element).shadowRoot!.querySelectorAll<HTMLElement>(".option")[1]?.click();
    await tick();
  ...
  it("delegates clear and focus APIs while preserving wrapper events", async () => {
    ...
    element.focus();
    expect(innerSelect(element).shadowRoot!.activeElement).toBe(selectTrigger(element));
    (innerSelect(element).shadowRoot!.querySelector(".clear") as HTMLElement).click();
    await tick();
  ...
  it("rebuilds options when a range prop changes", async () => {
    ...
    expect(innerSelect(element).shadowRoot!.querySelectorAll(".option")).toHaveLength(4);
  });
});
