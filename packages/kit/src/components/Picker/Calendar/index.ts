...
const elfCalendarCell = defineDirective(
  (element: HTMLElement, binding: DirectiveBinding<CalendarRenderValue>) => {
...
const resolvedFirstDayOfWeek = (): number => {
  ...
  return Number.isInteger(value) && value >= 0 && value <= 6 ? value : dateService.firstDayOfWeek;
};
...
const viewedSourceDate = (): Date =>
  parseDate(props.viewDate || props.defaultValue || selectedDate());
const hasSelectedValue = (): boolean =>
  Array.isArray(props.modelValue) ? props.modelValue.some(Boolean) : Boolean(props.modelValue);
...
const resolvedLocale = (): string => props.locale || dateService.context.locale || locale.name;
...
const days = (): DayCell[] => {
  ...
  const startOffset = (dateService.adapter.getWeekday(first) - resolvedFirstDayOfWeek() + 7) % 7;
  ...
  return Array.from({ length: 42 }, (_, index) => {
    ...
    return {
      ...
      inRange:
        Boolean(rangeStartValue && rangeEndValue) && iso > rangeStartValue && iso < rangeEndValue,
    };
  });
};
...
const renderDateCell = (cell: DayCell): CalendarRenderValue =>
  typeof props.renderDateCell === "function"
    ? props.renderDateCell(cell, dateFromCell(cell))
    : cell.label;
const cellClass = (cell: DayCell): string =>
  typeof props.cellClassName === "function"
    ? String(props.cellClassName(dateFromCell(cell)) || "")
    : "";
...
const Calendar = defineHtml<CalendarProps, Record<string, never>, CalendarSlots>(`
    <section class="calendar" part="calendar">
        ...
        <footer class="calendar-footer">
            <button type="button" class="today-button" @click=${() => {
              viewedDate.set(dateService.adapter.now());
              showDays();
            }}>${locale.t("calendar.today")}</button>
