...
describe("elf-descriptions", () => {
  ...
  it("shows named header slots without shortcut props and supports an empty slot", async () => {
    ...
    expect(
      root.querySelector<HTMLSlotElement>('slot[name="title"]')!.assignedElements(),
    ).toHaveLength(1);
    expect(
      root.querySelector<HTMLSlotElement>('slot[name="extra"]')!.assignedElements(),
    ).toHaveLength(1);
    expect(
      root.querySelector<HTMLSlotElement>('slot[name="empty"]')!.assignedElements(),
    ).toHaveLength(1);
  });
  ...
  it("keeps data and declarative horizontal label tracks aligned", () => {
    const parentCss = readFileSync(
      "packages/kit/src/components/Data/Descriptions/style.scss",
      "utf8",
    );
    const itemCss = readFileSync(
      "packages/kit/src/components/Data/DescriptionsItem/style.scss",
      "utf8",
    );
