...
const viewItems = (): ViewItem[] => {
  ...
  return source.map((entry, index) => {
    ...
    return {
      ...
      empty,
      span: Math.max(1, Math.min(effectiveColumn(), Math.floor(Number(raw[fields.span]) || 1))),
    };
  });
};
...
// Methods
const restoreChild = (child: DescriptionsItemElement, state: ManagedChildState): void => {
...
const Descriptions = defineHtml<DescriptionsProps, Record<string, never>, DescriptionsSlots>(`
