import type { TableAlign, TableColumnType, TableRow } from "./types";
...
const COLUMN_TYPES: TableColumnType[] = ["default", "selection", "index", "expand", "actions"];
...
export const getTableColumnSize = (
  ...
  widths[column.id] || tableSizeNumber(column.width || column.minWidth || "120px") || 120;
...
const defaultColumnLabel = (type: TableColumnType, prop: string, actionsLabel: string): string => {
...
export const normalizeTableColumns = (options: NormalizeTableColumnsOptions): TableColumnView[] => {
  ...
  const columns = options.columns.map((raw, index): TableColumnView => {
    ...
    return {
      ...
      align,
      headerAlign:
        raw.headerAlign === "center" || raw.headerAlign === "right" ? raw.headerAlign : align,
