...
export const normalizeTableFilterOptions = (column: TableSortFilterColumn): TableFilterOption[] => {
...
export const sortTableRows = (
  ...
  if (!column) {
    return data.sort(
      (left, right) =>
        compareTableValues(getTableValueAtPath(left, prop), getTableValueAtPath(right, prop)) *
        direction,
    );
  }
  return data
...
export const normalizeTableSortOrders = (column: TableSortFilterColumn): TableSortOrder[] => {
  const raw = Array.isArray(column.raw.sortOrders) ? column.raw.sortOrders : [];
  const normalized = raw
    .map((order): TableSortOrder => (order === "ascending" || order === "descending" ? order : ""))
