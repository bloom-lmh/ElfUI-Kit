...
// Derived state
const normalizedSize = (): EmptySize => (props.size === "compact" ? "compact" : "default");
...
const syncSlots = (): void => {
  host.shadowRoot?.querySelectorAll<HTMLSlotElement>("slot").forEach((slot) => syncSlotState(slot));
};
