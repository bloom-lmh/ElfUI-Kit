...
describe("elf-empty", () => {
  ...
  it("renders default slot actions", async () => {
    ...
    expect((el.shadowRoot!.querySelector(".bottom") as HTMLElement).style.display).not.toBe("none");
  });
  ...
  it("hides empty action and description regions without leaving layout gaps", async () => {
    ...
    expect((el.shadowRoot!.querySelector(".description") as HTMLElement).style.display).toBe(
      "none",
    );
  });
