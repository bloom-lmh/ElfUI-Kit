...
describe("virtual window", () => {
  ...
  it("keeps a 10,000-item virtual list DOM bounded while scrolling", async () => {
    ...
    expect(el.shadowRoot!.textContent).toContain("Item 8000");
    expect(el.shadowRoot!.querySelectorAll(".item").length).toBeLessThan(40);
    expect((el.shadowRoot!.querySelector(".window") as HTMLElement).style.transform).toBe(
      "translate3d(0, 319760px, 0)",
    );
    expect((el.shadowRoot!.querySelector(".window") as HTMLElement).style.top).toBe("0px");
  });
  ...
  it("recycles visible rows synchronously across continuous thumb-drag scroll events", async () => {
    ...
    expect(
      Array.from(el.shadowRoot!.querySelectorAll(".item")).some((item) =>
        initialElements.has(item),
      ),
    ).toBe(true);
  });
  ...
  it("uses estimated offsets for dynamic-height rows and updates after appending data", async () => {
    ...
    el.items = Array.from({ length: 100 }, (_, index) => ({
      id: index,
    ...
    el.items = [
      ...el.items,
      ...Array.from({ length: 20 }, (_, index) => ({ id: 100 + index, label: `Added ${index}` })),
    ];
    await tick();
  ...
  it("coalesces rapid dynamic scroll updates while keeping a buffered row window", async () => {
    const el = document.createElement("elf-virtual-list") as HTMLElement & Record<string, any>;
    el.items = Array.from({ length: 2000 }, (_, index) => ({
      id: index,
      label: `Dynamic ${index}`,
    }));
    el.dynamic = true;
    ...
    expect(
      Array.from(el.shadowRoot!.querySelectorAll(".scroll-window .item")).some((item) =>
        recycledElements.has(item),
      ),
    ).toBe(true);
    await frame();
    await tick();
  ...
  it("anchors dynamic measurements against the visible range instead of overscan", async () => {
    ...
    const el = document.createElement("elf-virtual-list") as HTMLElement & Record<string, any>;
    el.items = Array.from({ length: 200 }, (_, index) => ({
      id: index,
