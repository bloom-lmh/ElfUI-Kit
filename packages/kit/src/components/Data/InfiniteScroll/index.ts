...
const isExternalTarget = (): boolean =>
  normalizedContainer() != null && normalizedContainer() !== "";
...
const accessibleLabel = (): string => {
  ...
  return locale.name.toLowerCase().startsWith("zh") ? "无限滚动内容" : "Infinite scroll content";
};
...
useEffect(() => {
  const shouldFill = props.immediate && !props.disabled && !props.loading && !props.finished;
  void props.distance;
...
const InfiniteScroll = defineHtml<InfiniteScrollProps, InfiniteScrollEmits, InfiniteScrollSlots>(`
  <div
    class="scroll"
    part="scroll"
    role="region"
