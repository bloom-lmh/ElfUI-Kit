...
import { infiniteScrollDirective, registerInfiniteScrollDirective } from "./directive";
import type { InfiniteScrollDirectiveValue, InfiniteScrollExposes } from "./types";
...
describe("elf-infinite-scroll", () => {
  ...
  it("exposes check for retry and content-update workflows", async () => {
    ...
    document.body.appendChild(container);
    const el = document.createElement("elf-infinite-scroll") as HTMLElement &
      InfiniteScrollExposes & { container: HTMLElement; delay: number };
    el.container = container;
    el.delay = 0;
    const onLoad = vi.fn();
    el.addEventListener("load", onLoad as EventListener);
