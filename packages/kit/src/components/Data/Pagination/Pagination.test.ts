...
describe("elf-pagination", () => {
  it("keeps a stable pager window when a boundary page is selected", async () => {
    ...
    const pageSeven = Array.from(el.shadowRoot!.querySelectorAll<HTMLButtonElement>(".page")).find(
  ...
  it("修改 pageSize 触发 size-change", async () => {
    ...
    const option = Array.from(
      el.shadowRoot!.querySelectorAll<HTMLButtonElement>('[role="option"]'),
  ...
  it("uses default state only as the uncontrolled initial value", async () => {
    ...
    await tick();
    expect(
      el.shadowRoot!.querySelector<HTMLButtonElement>(".page.is-active")!.textContent?.trim(),
    ).toBe("2");
  });
  ...
  it("uses explicit page-count and exposes customized accessible navigation", async () => {
    ...
    expect(
      root.querySelector<HTMLButtonElement>(".arrow-left, .nav")?.getAttribute("aria-label"),
    ).toBe("Back");
    expect(root.querySelectorAll<HTMLButtonElement>(".nav")[1].getAttribute("aria-label")).toBe(
      "Forward",
    );
  });
  ...
  it("supports navigation icon props and SVG slots", async () => {
    ...
    expect(
      Array.from(el.shadowRoot!.querySelectorAll(".prop-icon"), (icon) => icon.textContent),
    ).toEqual(["←", "→"]);
