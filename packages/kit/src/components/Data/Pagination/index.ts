...
const pageItems = (): PagerItem[] => {
  ...
  if (start > 2)
    items.push({ key: "prev-more", label: "...", page: Math.max(1, start - 1), ellipsis: true });
  for (let pageNo = start; pageNo <= end; pageNo++) {
    items.push({ key: String(pageNo), label: String(pageNo), page: pageNo, ellipsis: false });
  }
  if (end < count - 1)
    items.push({ key: "next-more", label: "...", page: Math.min(count, end + 1), ellipsis: true });
