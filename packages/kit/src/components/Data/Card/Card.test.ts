...
describe("elf-card", () => {
  ...
  it("forwards image alt text and exposes a localized error state", async () => {
    ...
    expect(el.shadowRoot!.querySelector(".image-error")?.getAttribute("aria-label")).toBe(
      "图片暂时无法显示",
    );
  ...
  it("cover 插槽接受没有文本内容的图片元素", async () => {
    ...
    expect(
      el.shadowRoot!.querySelector<HTMLSlotElement>('slot[name="cover"]')?.assignedElements(),
    ).toContain(image);
  });

  it("includes a reduced-motion fallback for cover and loading animation", () => {
    const cssText = readFileSync("packages/kit/src/components/Data/Card/style.scss", "utf8");
