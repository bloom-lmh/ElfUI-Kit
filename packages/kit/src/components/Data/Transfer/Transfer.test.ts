...
describe("elf-transfer", () => {
  it("renders source, target, and action controls", async () => {
    ...
    expect(
      Array.from(el.shadowRoot!.querySelectorAll(".buttons button"), (button) =>
  ...
  it("moves checked source items and emits update and change payloads", async () => {
    ...
    el.addEventListener(
      "update:modelValue",
      (event: Event) => (update = (event as CustomEvent).detail),
    );
  ...
  it("does not move disabled items and reports checked-key changes", async () => {
    ...
    el.addEventListener(
      "left-check-change",
      (event: Event) => (checked = (event as CustomEvent).detail),
    );
  ...
  it("applies default checked keys only to selectable items", async () => {
    ...
    const toRight = el.shadowRoot!.querySelector(
  ...
  it("supports unshift target order with controlled model writeback", async () => {
    ...
    (el.shadowRoot!.querySelector(".buttons button:first-child") as HTMLButtonElement).click();
    await tick();
    const labels = Array.from(el.shadowRoot!.querySelectorAll(".panel-right .panel-item span")).map(
  ...
  it("shows the empty state after a filter has no matches", async () => {
    ...
    expect(el.shadowRoot!.querySelector(".panel-left .panel-empty")?.textContent).toContain(
      "Nothing matched",
    );
  });
  ...
  it("renders typed custom content with side and selection context", async () => {
    const el = await mount((transfer) => {
      transfer.renderContent = (
        item: { label: string },
        context: { side: string; checked: boolean },
      ) => {
    ...
    expect(el.shadowRoot!.querySelector(".panel-left .item-content")?.textContent).toBe(
      "left:Item 1:idle",
    );
