...
describe("tree collection", () => {
  ...
  it("projects expansion and filtered ancestor paths without mutating data", () => {
    ...
    expect(collection.visible(["root"], "").map((row) => row.key)).toEqual([
      "root",
      "enabled",
      "disabled",
      "other",
    ]);
    expect(collection.visible([], "target").map((row) => row.key)).toEqual(["other", "needle"]);
  ...
  it("cascades checks while excluding disabled nodes and supports strict mode", () => {
    ...
    expect(collection.normalizeChecked(["root"], false)).toEqual(["root", "enabled"]);
    expect(collection.normalizeChecked(["enabled"], false)).toEqual(["enabled", "root"]);
    expect(collection.normalizeChecked(["root"], true)).toEqual(["root"]);
    expect(collection.normalizeChecked(["root"], false, true)).toEqual([]);
  });
});
