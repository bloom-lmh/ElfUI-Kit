...
describe("elf-tree", () => {
  it("默认无边框并支持显式 bordered 外观", async () => {
    ...
    expect(bordered.shadowRoot!.querySelector(".tree")!.classList.contains("is-bordered")).toBe(
      true,
    );
  });
  ...
  it("supports append, remove, and relative insertion without replacing the tree", async () => {
    ...
    await tick();
    expect(
      Array.from(el.shadowRoot!.querySelectorAll(".tree-label"), (node) => node.textContent),
    ).toEqual(["Root", "A", "B", "C", "D"]);
  ...
  it("virtualizes large flat trees and scrolls to a public key", async () => {
    const el = document.createElement("elf-tree") as TreeEl;
    el.data = Array.from({ length: 2000 }, (_, index) => ({
      key: `node-${index}`,
      label: `Node ${index}`,
    }));
    el.virtual = true;
  ...
  it("auto expands ancestors and checks a leaf when its content is clicked", async () => {
    ...
    const leaf = Array.from(el.shadowRoot!.querySelectorAll<HTMLElement>(".tree-content")).find(
      (node) => node.textContent?.includes("Leaf"),
    )!;
    leaf.click();
    await tick();

    expect((onChecked.mock.calls[0]![0] as CustomEvent<string[]>).detail).toEqual(
      expect.arrayContaining(["root", "branch", "leaf"]),
    );
  });
  ...
  it("supports renderContent, field classes, the empty slot, and accessibility options", async () => {
    ...
    expect(el.shadowRoot!.querySelector(".tree-label strong")?.textContent).toBe(
      "Release · level 1",
    );
    expect(el.shadowRoot!.querySelector(".tree")!.getAttribute("aria-label")).toBe("Release tree");
    expect(
      el.shadowRoot!.querySelector(".tree")!.classList.contains("is-scrollbar-always-on"),
    ).toBe(true);
      ...
      tree.data = [
        {
          key: "source",
          label: "源目录",
          children: [{ key: "asset", label: "设计资源", isLeaf: true }],
        },
        {
          key: "target",
          label: "目标目录",
          children: [{ key: "placeholder", label: "占位资源", isLeaf: true }],
        },
      ];
