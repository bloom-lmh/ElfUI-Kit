...
## 2026-07-28 架构与双语文档收敛

- [x] 抽取不可变 `tree-collection`，统一节点规范化、键索引、父子关系、展开投影、过滤和复选传播。
- [x] Tree 主组件只协调响应式状态、DOM、懒加载、虚拟窗口和拖拽事件，不再重复扫描原始数据建立关系。
- [x] 为 collection 增加纯逻辑特征测试，覆盖字段映射、祖先展开、过滤路径、严格与级联复选。
- [x] 7 个案例（8 个 Playground）及 API 表完整接入 Provider 文档语言，标题、状态、按钮、数据、占位符和 ARIA 同步切换。
- [x] 删除案例外层重复小标题，目录只采用简短 Playground 标题，并保持中英文语义一一对应。
