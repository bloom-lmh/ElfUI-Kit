...
import { draggableDirective, type DraggableOptions } from "../../../directives/draggable";
...
const rebuildVisible = (): void => {
  visibleNodes.set(
    collection.peek().visible(expandedState.peek(), filterText.peek(), resolvedFilterMethod()),
  );
};

const setExpandedKeys = (keys: TreeKey[], shouldEmit = true): void => {
  const next = collection.peek().normalizeExpanded(normalizeKeys(keys), props.autoExpandParent);
...
const numericHeight = (): number =>
  Math.max(itemSize(), Number.parseFloat(String(props.height)) || 420);
const virtualWindow = () =>
  ...
  if (open) {
    if (props.accordion) {
      const siblings = new Set(
        collection
          .peek()
          .rows.filter((node) => node.parentKey === row.parentKey && node.hasChildren)
          .map((node) => node.key),
      );
...
const loadChildren = async (row: TreeViewNode): Promise<void> => {
  if (!props.lazy || typeof props.load !== "function" || loadedKeys.peek().includes(row.key))
    return;
...
const toggleExpand = async (row: TreeViewNode): Promise<void> => {
  ...
  if (
    !isExpanded(row.key) &&
    props.lazy &&
    !loadedKeys.peek().includes(row.key) &&
    childRowsOf(row).length === 0
  ) {
...
const getHalfCheckedKeys = (): string[] =>
  collection
    .peek()
    .rows.filter((row) => isIndeterminate(row))
    .map((row) => row.key);
...
const appendNode = (data: TreeNode, parent: TreeNode | string | number | null = null): void => {
  ...
  if (parent == null) source.push(data as Record<string, unknown>);
    ...
    (location.node[field.children] as Record<string, unknown>[]).push(
...
const insertRelative = (
  data: TreeNode,
  reference: TreeNode | string | number,
  offset: 0 | 1,
): void => {
...
const insertBeforeNode = (data: TreeNode, reference: TreeNode | string | number): void =>
  insertRelative(data, reference, 0);
const insertAfterNode = (data: TreeNode, reference: TreeNode | string | number): void =>
  insertRelative(data, reference, 1);
...
const onNodeClick = (row: TreeViewNode): void => {
  ...
  select(row.key);
  const shouldCheck =
    props.showCheckbox && (props.checkOnClickNode || (props.checkOnClickLeaf && row.isLeaf));
  ...
  if (keyboardDragging.peek()) {
    ...
    if (["ArrowUp", "ArrowDown", "ArrowRight"].includes(event.key)) {
      ...
      const currentIndex = Math.max(
        0,
        rows.findIndex((item) => item.key === dropTargetKey.peek()),
      );
      ...
      const target = direction
        ...
        : rows[currentIndex];
      const dragging = findNode(draggingKey.peek());
      const placement: TreeDropType =
        event.key === "ArrowRight" ? "inner" : direction < 0 ? "before" : "after";
...
const canDrag = (row: TreeViewNode): boolean =>
  ...
  (typeof props.allowDrag !== "function" || Boolean(props.allowDrag(row.raw as TreeNode)));

const canDropOnRow = (
  dragging: TreeViewNode,
  row: TreeViewNode,
  placement: TreeDropType = "inner",
): boolean =>
...
const onDragLeave = (row: TreeViewNode, event: Event): void => {
  ...
  if (dragEvent.relatedTarget instanceof Node && currentTarget.contains(dragEvent.relatedTarget))
    return;
...
const treeDragOptions = (row: TreeViewNode): DraggableOptions<TreeViewNode> => ({
  ...
  mode: "sort",
  canDrop: ({ source, placement }) =>
    canDropOnRow(source.data, row, placement === "inside" ? "inner" : placement),
});
