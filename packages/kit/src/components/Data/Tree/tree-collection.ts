...
export const buildTreeCollection = (
  ...
  const prune = (keys: readonly string[]): string[] => keys.filter((key) => Boolean(byKey[key]));

  const normalizeExpanded = (keys: readonly string[], autoExpandParent: boolean): string[] => {
  ...
  const normalizeChecked = (
    ...
    for (const key of prune(keys)) {
      ...
      const affected =
        !checkStrictly && row.hasChildren && !leafOnly ? descendantsOf(row, true) : [row];
  ...
  const cascadeChecked = (keys: readonly string[], checkStrictly: boolean): string[] => {
