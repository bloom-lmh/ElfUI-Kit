...
describe("elf-locale-provider", () => {
  ...
  it("嵌套 provider 只覆盖自己的子树", async () => {
    ...
    expect(outer.querySelector("#inner-probe")!.shadowRoot?.textContent).toContain(
      "zh-CN|ltr|确认",
    );
  });
  ...
  it("格式化数字和日期时使用当前 locale 与时区", async () => {
    ...
    const text =
      provider.querySelector("elf-locale-provider-format-probe")!.shadowRoot?.textContent || "";
  ...
  it("委托外部 i18n adapter 并为缺失键保留内置回退", async () => {
    ...
    provider.adapter = defineLocaleAdapter({
      translate(path, _params, context) {
        return path === "common.confirm" ? `Valider · ${context.name}` : undefined;
      },
    ...
    expect(provider.querySelector("elf-locale-provider-probe")?.shadowRoot?.textContent).toContain(
      "fr-FR|ltr|Valider · fr-FR",
    );
    const formatted =
      provider.querySelector("elf-locale-provider-format-probe")?.shadowRoot?.textContent ?? "";
    expect(formatted).toContain("NOMBRE EXTERNE");
    expect(formatted).toContain("DATE EXTERNE");
    expect(provider.querySelector("elf-select")?.shadowRoot?.textContent).toContain("Select");
  });
});
