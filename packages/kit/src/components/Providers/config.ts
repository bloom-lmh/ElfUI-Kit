...
const mergeValue = (left: unknown, right: unknown, key = ""): unknown => {
...
export const mergeElfUIConfig = (...sources: unknown[]): ElfUIConfig =>
  sources.reduce<ElfUIConfig>((output, source) => mergeValue(output, source) as ElfUIConfig, {});
...
export const useDisplayProvider = (): DisplayProviderContext => useConfigProvider().display;
