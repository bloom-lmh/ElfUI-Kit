...
export const EN_LOCALE_MESSAGES: LocaleMessages = {
  ...
  upload: {
    ...
    directoryHint:
      "Folder upload depends on browser directory selection support; use multi-file upload as a fallback.",
  },
  tour: {
    close: "Close tour",
  },
...
export const localeMessagesFor = (name: string): LocaleMessages =>
  String(name).toLowerCase().startsWith("zh") ? DEFAULT_LOCALE_MESSAGES : EN_LOCALE_MESSAGES;
...
const readDefaultLocaleMessages = (): LocaleMessages => localeMessagesFor(readDocumentLocaleName());
