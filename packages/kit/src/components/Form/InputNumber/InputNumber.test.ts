...
describe("elf-input-number", () => {
  ...
  it("does not render increment controls when controls is false", async () => {
    ...
    expect(
      el.shadowRoot!.querySelector(".input-number")?.classList.contains("controls-hidden"),
    ).toBe(true);
  });
  ...
  it.each(["default", "stacked", "split", "hidden"] as const)(
    ...
    async (controlVariant) => {
      ...
      expect(
        el
          .shadowRoot!.querySelector(".input-number")
          ?.classList.contains(`controls-${controlVariant}`),
      ).toBe(true);
      expect(el.shadowRoot!.querySelectorAll(".control")).toHaveLength(
        controlVariant === "hidden" ? 0 : 2,
      );
    },
  );
