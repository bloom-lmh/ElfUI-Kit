...
const prefixes = (): string[] => {
  if (Array.isArray(props.prefixes) && props.prefixes.length)
    return props.prefixes.filter(Boolean).sort((a, b) => b.length - a.length);
...
const parseQuery = (value: string, caret: number): void => {
  ...
  for (const prefix of prefixes()) {
    ...
    if (
      String(props.split || " ")
        .split("")
        .some((separator) => pattern.includes(separator))
    )
      continue;
...
const selectAt = (index: number): void => {
  ...
  queueMicrotask(() => {
    ...
    if (!textarea) return;
    const position =
      currentRange.start + currentRange.prefix.length + option.insertText.length + suffix.length;
    textarea.focus();
