...
const normalizedMaxHeight = (): number =>
  Math.max(normalizedItemHeight(), Number(props.maxHeight) || 280);
...
const optionsViewportStyle = (): Record<string, string> => ({
  ...
  ...(virtualEnabled()
    ? { height: `${Math.min(options().length * normalizedItemHeight(), normalizedMaxHeight())}px` }
    : {}),
});
...
const optionStyle = (item: ViewOption): Record<string, string> =>
  virtualEnabled()
    ? {
        position: "absolute",
        top: `${item.index * normalizedItemHeight()}px`,
...
const placement = (): AutocompletePlacement =>
...
const getInputEl = (): HTMLInputElement | null =>
  host.shadowRoot?.querySelector<HTMLInputElement>("input") || null;
...
const Autocomplete = defineHtml<AutocompleteProps>(`
  <div
    class="autocomplete"
    part="autocomplete"
    :class=${[`placement-${placement()}`, { loading: isLoading() }]}
    ...
    >
      ...
      <template v-else>
        ...
        >
          <div
            class="options-track"
            :style=${virtualEnabled() ? virtualTrackStyle() : {}}
          >
            <button
