...
const optionsTrackStyle = (): Record<string, string> =>
  virtualEnabled() ? { height: `${virtualWindow().totalSize}px` } : {};
...
const clear = (): void => {
  const next = fieldValues.valueOnClear<SelectValue | SelectValue[]>(props.valueOnClear, () =>
    isMulti() ? [] : "",
  );
  innerValue.set(next);
  emit("update:modelValue", next);
