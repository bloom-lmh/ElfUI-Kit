import type { TreeCollection, TreeFieldConfig } from "../../Data/Tree/tree-collection";
...
export const normalizeTreeSelectKeys = (
  ...
  return source
    .filter((item): item is TreeSelectValue => typeof item === "string" || typeof item === "number")
    .map(String)
    .filter(Boolean);
};
...
export const treeSelectEntry = (
  ...
  const value = typeof rawValue === "string" || typeof rawValue === "number" ? rawValue : key;
  return {
    key,
    value,
...
): TreeSelectEntry[] => keys.map((key) => treeSelectEntry(collection, fields, key));
