...
const optionTreeDepth = (options = rawOptions()): number =>
  options.reduce(
    (depth, option) => Math.max(depth, 1 + optionTreeDepth(optionChildren(option))),
    0,
  );
...
const matchesFilter = (node: CascaderNodeSnapshot, keyword: string): boolean => {
  try {
    return Boolean(
      ((props.filterMethod as CascaderFilterMethod | undefined) ?? defaultFilter)(node, keyword),
    );
  } catch {
    return false;
  }
...
const removeTag = (event: Event): void => {
  ...
  const next = selectedValues
    ...
    .filter(
      (item) =>
        !samePathValue(item, entry.value) && !covered.some((leaf) => samePathValue(item, leaf)),
    );
  selectedValues.set(next);
  emitChange(next);
...
const focusableOptions = (): HTMLButtonElement[] =>
  Array.from(
    host.shadowRoot?.querySelectorAll<HTMLButtonElement>(".dropdown button:not(:disabled)") || [],
  );
...
const columnOptions = (option: HTMLButtonElement): HTMLButtonElement[] =>
  Array.from(
    option.closest(".column")?.querySelectorAll<HTMLButtonElement>(".option:not(:disabled)") || [],
  );
...
const treeOptionButtons = (): HTMLButtonElement[] =>
  Array.from(
    host.shadowRoot?.querySelectorAll<HTMLButtonElement>(".tree-option:not(:disabled)") || [],
  );
...
const popperOffset = (): [number, number] => popperModifier("offset")?.options?.offset || [0, -1];
