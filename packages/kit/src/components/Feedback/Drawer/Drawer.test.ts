...
describe("elf-drawer", () => {
  it("通过 Core Transition 管理结构生命周期和 reduced-motion 样式", () => {
    const source = readFileSync("packages/kit/src/components/Feedback/Drawer/index.ts", "utf8");
    const cssText = readFileSync("packages/kit/src/components/Feedback/Drawer/style.scss", "utf8");
