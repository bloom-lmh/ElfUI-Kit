...
const dismissibleOverlay = useDismissibleOverlay({
  kind: "tooltip",
  containers: () => [host, host.shadowRoot?.querySelector<HTMLElement>(".tooltip-content")],
  closeOnEscape: () => true,
  closeOnOutside: () =>
    props.trigger === "click" || props.trigger === "contextmenu" || touchOpen.peek(),
  onRequestClose: () => hide(),
});
...
const tooltipStyle = (): Record<string, string> => ({
  "--elf-tooltip-max-width":
    typeof props.maxWidth === "number"
      ? `${Math.max(80, props.maxWidth)}px`
      : String(props.maxWidth || "240px"),
});
