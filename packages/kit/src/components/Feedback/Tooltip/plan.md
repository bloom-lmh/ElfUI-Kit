...
## 2026-08-05 箭头改为旋转方块方案

- [x] 用户反馈「悬停或聚焦看得到内部的三角形」：原箭头用双三角形（`::before` 描边 + `::after` 填充）拼接，在气泡边缘产生可见接缝/内部三角形；改为与 PopConfirm 一致的旋转方块方案（10×10、`background: inherit`、`border: inherit` 并裁掉两条边），四个方向用 `rotate(225/45/135/-45deg)` 定位，深色/浅色气泡均无缝衔接。
- [x] 深色气泡由半透明 `rgba(33,33,33,0.9)` 改为不透明 `#212121`：旋转方块朝内的角会继承气泡背景，半透明背景会让内角在气泡内叠出可见的深色小三角，不透明后内角与气泡同色、被完全覆盖；PopConfirm 使用不透明 `--elf-bg-paper` 故无此问题。
- [x] 删除 dark/light 两组伪元素箭头配色规则与各方向伪元素偏移；`part="arrow"` 对外结构不变。
- [x] 验证：Tooltip 组件与 TooltipPage 聚焦测试 19/19 通过；Prettier 通过。
