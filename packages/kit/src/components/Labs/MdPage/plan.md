...
## 2026-08-05 安装页 Markdown 改造

- [x] 新增“Installation 2 安装 2”页面（`/getting-started/installation-md`）：用 `elf-md-page` 以 Markdown 驱动整个安装指南。
- [x] 代码卡片使用 `elf-code-card`（含 tab 与行号关闭），表格使用 `elf-table`，提示使用 `elf-quote`，全部以原始 HTML 嵌入 Markdown；运行时 JSON 属性解析与布尔 `"false"` 转换验证通过。
- [x] 原安装页保留未动；路由、导航、图标与信息架构测试同步。

## 2026-08-05 安装页反馈修复

- [x] 内容列宽度 716 → 748（去掉多余内边距，占满路由出口）。
- [x] 取消标题下的重复 quote，改为普通引导段落；仅保留报告/自定义提示框。
- [x] 修复 HTML / Macro 代码卡不显示：多行代码改走 `items` JSON 属性（保留真实换行），嵌入组件统一包 `<div>` 保证 markdown-it 块级解析。
- [x] “第一个组件”改为组合案例：项目名输入框 + 状态标签 + 创建/取消按钮。

## 2026-08-05 安装页宽度与响应式

- [x] 内容列与页头严格对齐：放回 `docs-article` 容器，沿用 `max(85%, min(100%, 900px))` 宽度公式（1440 下 hero / 内容均为 876px）。
- [x] 嵌入组件样式移入 Markdown 内的 `<style>`（作用于 md-page 阴影），修复页面级 CSS 无法穿透的问题。
- [x] 响应式：390px 无横向溢出，演示卡按钮换行，表格/代码卡内部滚动。

## 2026-08-05 编辑器窗口外观与全屏宽对齐

- [x] 代码卡改为 `elf-code-card` 的 `window`（编辑器窗口）外观（红绿灯 + 标题栏）。
- [x] md-page 去掉 900px 上限（`max-width="100%"`），内容列与页头在 1440 / 1920 下均严格等宽（876 / 993）。

## 2026-08-05 window 变体 tab 修复与卡片案例

- [x] 修复 CodeCard window 变体带 tab 时头部网格仍为 `96px 1fr auto`，导致 tab 坍缩到 96px 的缺陷；`group-header` 改为 `minmax(0,1fr) auto`。
- [x] “第一个组件”改为 `elf-card` 组合案例：标题/副标题 + 就绪标签 + 项目名输入框 + 创建/取消按钮 + 页脚说明。

## 2026-08-05 远程案例全宽

- [x] “远程文件与链接拦截”案例改单列布局（移除遗留的双列网格），md-page 加 `max-width="100%"`，面板与内容撑满 Playground 宽度。

## 2026-08-05 安装页代码卡去页脚

- [x] 安装 2 页代码卡移除 footer（window 变体标题栏已含文件名，页脚冗余），6 张卡统一无页脚。
