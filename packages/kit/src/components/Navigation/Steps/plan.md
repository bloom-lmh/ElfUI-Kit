...
- [x] 2026-07-21：参考 Vuetify Stepper 增加集成内容卡片、前后操作、本地化文字、隐藏操作、editable、alt-labels 和移动端纵向布局，并新增可操作配置台。

## 2026-08-05 步骤内容内边距

- [x] `.step-main`（每个步骤的标题与描述内容块）由仅 `padding-inline-end: 10px` 改为 `padding: 6px 10px`，为每个步骤的内容增加上下呼吸空间，水平保持与图标/连接线的间隔。
- [x] 验证：Steps 组件与 StepsPage 聚焦测试 15/15 通过；Prettier 通过。浏览器截图受本机 Playwright CLI 会话故障影响未能归档。
