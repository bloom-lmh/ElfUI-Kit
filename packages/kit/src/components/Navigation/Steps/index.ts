...
const rawItems = (): StepItem[] =>
  hasStepChildren.value
    ? childItems()
    : Array.isArray(props.items)
      ? (props.items as StepItem[])
      : [];
...
const syncStepChildren = (): void => {
  ...
  children.forEach((child, index) => {
    ...
    child.clickable =
      !child.disabled && Boolean(props.clickable || props.editable || !props.linear);
