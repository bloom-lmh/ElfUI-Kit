...
- [x] 命令控制按钮迁移到 Playground 标题操作区
- [x] 消息列表示例约束滚动宿主宽度，内容与垂直轨道仅保留 8px 稳定 gutter，不再出现大块空白。
- [x] 1200px、360px 与暗色主题真实浏览器验证无页面溢出；命令控制到顶/到底精确命中 0 与最大滚动值。
...
## 2026-08-05 滚动位置命令按钮位置调整

- [x] 「滚动位置命令」案例的「回到顶部 / 滚动到底部」按钮与「当前位置」状态从 Playground 顶部状态槽移到滚动列表下方，形成操作栏紧贴内容的布局；`cmd-row` 增加间距与状态文字样式。
- [x] 验证：ScrollbarPage 聚焦测试通过；Chromium 实测按钮在滚动区域下方且功能不变（到底 800px / 回顶 0px）；截图归档 `output/playwright/scrollbar-commands-below.png`。
